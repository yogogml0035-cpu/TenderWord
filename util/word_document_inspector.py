...
class WordDocumentInspector:
    ...
    def _build_paragraph_context_by_index(self, paragraphs: any, idx: int, total: int) -> str:
        """
    ...
    def inspect_comments(
        ...
        if use_range:
            print(f"{log_prefix}仅抽取批注范围: [{range_start}, {range_end})")
                ...
                try:
                    ...
                    if scope_obj is not None:
                        try:
                            ...
                            reference_text = ""
                        scope_text = self._build_scope_with_neighbor_paragraphs(scope_obj)
    ...
    def _check_paragraph_strikethrough(
        ...
        try:
            if not (hasattr(range_obj.Font, "StrikeThrough") and range_obj.Font.StrikeThrough):
                return None
        ...
        try:
            ...
            for i in range(1, total_paragraphs + 1):
                try:
                    ...
                    print(f"{log_prefix}检测段落 {i} 时出错: {e}")
                    continue
    ...
    def analyze_document(
        ...
        strikethroughs, non_black_fonts = self._inspect_paragraphs_strikethrough_and_non_black(
            range_start=range_start, range_end=range_end
        )
