    ...
    try:
        ...
        if os.path.exists(cache_dir):
            ...
            try:
                ...
                logger.debug(
                    f"[win32com缓存] 重置 gencache 状态时出错 (可忽略): {reset_e}"
                )
...
def _create_word_application_internal(
    ...
    if initial_delay > 0:
        logger.debug(
            f"{log_prefix}等待 {initial_delay} 秒后创建 Microsoft Word 实例..."
        )
    ...
    for attempt in range(MAX_RETRIES + 1):
        try:
            ...
            if is_rpc_error(e) and attempt < MAX_RETRIES:
                ...
                logger.warning(
                    f"{log_prefix}创建 Word 失败 (尝试 {attempt + 1}/{MAX_RETRIES + 1}): {e}"
                )
...
def _try_create_word_app(
    use_existing: bool, node_name: str, cache_cleared: bool = False
) -> any:
    """
    尝试创建 Word 应用程序实例
    ...
    if word_app is None and last_exception is not None:
        if not cache_cleared and is_gencache_error(last_exception):
            ...
            if clear_win32com_cache():
                ...
                logger.error(
                    f"{log_prefix}清除缓存失败，请手动删除 win32com gen_py 缓存目录"
                )
        ...
        if DIAGNOSTICS_AVAILABLE:
            try:
                ...
                if not word_check.get("installed"):
                    ...
                    error_msg += (
                        f"  COM ProgID: {word_check.get('com_progid', 'Unknown')}\n\n"
                    )
                    error_msg += "【可能的原因】\n"
                    error_msg += (
                        "  1. Word 版本不兼容（建议使用 Office 2016 或更高版本）\n"
                    )
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            if is_rpc_error(e) and attempt < max_retries:
                delay = calculate_retry_delay(attempt)
                logger.warning(
                    f"{log_prefix}打开文档失败 (尝试 {attempt + 1}/{max_retries + 1}): {e}"
                )
                logger.info(f"{log_prefix}等待 {delay:.1f} 秒后重试...")
                time.sleep(delay)
            else:
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if is_rpc_error(e) and attempt < max_retries:
                    delay = calculate_retry_delay(attempt)
                    logger.warning(
                        f"{log_prefix}保存文档失败 (尝试 {attempt + 1}/{max_retries + 1}): {e}"
                    )
                    logger.info(f"{log_prefix}等待 {delay:.1f} 秒后重试...")
                    time.sleep(delay)
                else:
