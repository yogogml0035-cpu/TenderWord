...
15. 结构化表审核总则：若【技术参数】中一组连续表格行后紧跟 `[[TABLE:<id>]]`，这组表格行只是结构化表投影预览，`[[TABLE:<id>]]` 才是真实表写回锚点。待审核正文必须保留该锚点独占一行，并且不得把锚点前投影表行重复转写成普通正文、编号列表、纯文本表格或手绘表格。如果待审核正文在该位置只保留必要章节标题、表前表后非表格正文和独占一行的 `[[TABLE:<id>]]`，这是通过项，必须忽略，不得以“缺少投影表行”输出 finding。典型错误：技术参数含 `| 1 | 设备用途 | ... [[TABLE:TP1_1]]`，待审核正文写成 `1 设备用途`、`1.1 用于...`、`2 配置要求`，同时又保留 `[[TABLE:TP1_1]]`；这种“锚点 + 散文化投影表”必须输出 finding，要求删除散文化投影行，保留必要章节标题、该表前后非表格正文和 `[[TABLE:TP1_1]]`。若该锚点表属于评分污染，则标题、投影表和锚点都应删除。
16. 结构化表邻接正文审核总则：`[[TABLE:<id>]]` 只代表紧邻它之前的一张结构化表，不代表表格前后的附件标题、报价说明、注释、签字盖章、日期、后续章节或其它非表格正文。审核时必须逐段检查【技术参数】中每个非评分锚点前后的非表格行：如果待审核正文把多个锚点集中到文末、连续输出多个 `[[TABLE:...]]` 后结束，或缺失两个锚点之间/锚点之后的真实非表格正文，必须输出 finding 要求按技术参数原位置恢复这些非表格正文；不要要求重画表格，不要恢复评分表。
...
# 确定性硬契约提示：[[TABLE:id]] 是内部结构化写回入口，是真实表格还原进 Word 的唯一锚点。
TABLE_PLACEHOLDER_CONTRACT_PROMPT = (
    "【结构化表占位符硬契约】技术参数中的 `[[TABLE:<id>]]` 是结构化表的内部写回入口，属于运行时硬契约，是真实表格还原进 Word 的唯一锚点。\n"
    "- 经写回层处理后，`[[TABLE:id]]` **不应作为可见行**出现在最终 Word 里：写回层会按 id 把它替换成真实表格，或在无法恢复时静默丢弃。\n"
    "- 因此待审核正文里保留 `[[TABLE:id]]` 占位符是**正确且被要求**的行为：**不要**因为正文中出现 `[[TABLE:id]]` 就输出 finding 要求删除它，也不要把占位符当成需要清理的脏数据或多余内容。\n"
    "- **必须检查锚点表是否被重复散文化**：如果【技术参数】中某个 `[[TABLE:id]]` 前紧邻的是表格投影，而【待审核正文】除了保留 `[[TABLE:id]]` 外，还把同一表格投影内容转写成普通文本、编号列表、Markdown/纯文本表格或手绘表格，必须输出 finding 要求删除这些重复投影内容，只保留锚点。\n"
    ...
    "- 但如果该结构化表所在采购需求正文仍被保留，且待审核正文用 Markdown 表格、手绘表格、普通列表或散文参数替代了 `[[TABLE:id]]`，这不是“缺失占位符”，而是表格投影被错误可见化，必须输出 finding，要求删除可见投影内容并恢复独占一行的 `[[TABLE:id]]`。\n"
    "- **不要**把 Markdown 表格、手绘表格或散文式参数列表当作占位符的等价物；有锚点时它们是重复投影内容，不是正确替代品。\n"
    "- 无锚点普通表仍按技术参数表格 schema 审核；有锚点表只审核锚点保留、投影不重复、评分污染删除。\n"
...
def _render_verify_user_prompt(
    *,
    ...
    return (
        ...
        "- 结构化表必须按锚点审核：若【技术参数】中连续 `|...|` 表格行后紧跟 `[[TABLE:<id>]]`，这些表格行是投影预览，不是正文条款；【待审核正文】只允许用该锚点独占一行代表表格本体。若待审核正文把同一投影行改成 `1 设备用途`、`1.1 ...`、编号列表、Markdown/纯文本表格或手绘表格，必须输出 finding，要求删除这些重复投影内容，保留必要章节标题、表前表后非表格正文和 `[[TABLE:<id>]]`。\n"
        "- 如果待审核正文在结构化表位置只保留必要章节标题、表前表后非表格正文和独占一行的 `[[TABLE:<id>]]`，且没有重复投影行，这是正确行为，必须忽略，不得输出 `fix_hint` 为“无”的 finding。\n"
        "- 结构化表占位符不得吞掉正文：逐段检查【技术参数】中每个非评分锚点前后是否有附件标题、报价说明、注释、签字盖章、日期或后续章节等非表格正文；若这些正文在【待审核正文】缺失，或多个 `[[TABLE:...]]` 被集中移到文末连续输出，必须输出 finding 要求恢复这些非表格正文并把锚点放回原位置。不要要求恢复评分表，不要要求重画结构化表。\n"
