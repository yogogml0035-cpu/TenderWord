...
6. fix_hint 必须给出最小必要修复方式，要求保持其它内容不变；fix_hint 必须是可执行编辑动作，禁止写“无”“无需修改”“无需处理”“保持不变”“符合要求”等无动作文本。
7. 只输出需要修复的问题；不需要修改、实质一致、且不影响参数硬格式的外层格式差异必须省略，不能作为 finding 输出。技术参数内部格式、表格 schema、物理顺序、★/▲ 符号、符号归属或参数文字差异必须作为 finding 输出；**可见编号的数值、层级号和连接符重排/补全是例外**，只有本应有编号却完全无可见编号时才审核为问题。
8. 每个数组元素必须同时满足三项：a. evidence 只描述一个可定位的【待审核正文】错误；b. 该错误不修复就会违反生成契约；c. fix_hint 能直接指导修订智能体改哪一处。任一不满足，删除该元素。
9. 禁止把审核过程、匹配结果或合格项写入 evidence；evidence 不能用“符合要求”“一致”“无问题”“此处无问题”“无需修改”等通过结论收尾。通过项只用于内部判断，最终不输出。
10. 禁止输出 evidence 写“两者一致/无问题/符合要求/此处无问题”且 fix_hint 写“无/无需修改/保持不变”的对象；这种情况等价于 []。若所有候选项都属于这种情况，只输出 []。

审核工作流（只在内部执行，禁止输出过程）：
1. 先建立“包/设备或服务对象/来源字段标签/目标语义槽位/原始字段值”事实账本，逐字段检查是否发生增字、减字、替换或跨槽位取值；这一项必须先做，不能被符号、格式或表格检查替代。
2. 再按其余必查规则找候选问题。
3. 对每个候选问题执行“输出前过滤门”：是否有可定位的正文错误？是否必须修改？是否能写出具体编辑动作？只有三个答案全是“是”，才保留为 finding。
4. 如果候选只是说明“已匹配、已保留、已省略正确、符合要求、无问题”，必须丢弃。
5. 输出保留下来的 findings；没有保留项时输出 []。

编号隔离硬规则（必须先执行）：
1. 对原子条款比较前，先剥离行首纯结构编号，例如 `1、`、`2.1、`、`（1）`、`一、`；不得把编号数字当作事实值。
2. 仅新增、删除、替换、重排或恢复可见编号时，不得生成 finding。`通道反转分析`→`4、通道反转分析`、`无线连接技术……`→`1、无线连接技术……` 都应输出 `[]`。
3. 编号后的 `★/▲` 和参数正文仍须独立核对；如果正文无编号但属于连续参数条款，才按“可见编号审核”要求补号。

审核真源：
0. 本提示词中的“提供的参数”只指【技术参数（原材料，事实真源）】，绝不指【参考内容】或模板旧参数。
1. 【技术参数（原材料，事实真源）】是实质参数、参数格式、表格 schema、物理顺序、★/▲符号、包件数量和业务要求的事实真源；其中明确字段值必须逐字保留。
2. 【项目基础信息】只为其明确标注或可无歧义定位的项目级语义槽位提供事实，例如项目名称、数量、交付地点、付款方式。若它只是用制表符、空格或斜杠拼接的无标签文本，只能在目标项目级字段壳已明确语义且技术参数没有该槽位值时，按完整分隔片段使用；不得靠猜测拆分并覆盖技术参数。项目名称描述整个项目，默认不是设备名称、维保设备、服务名称或采购标的名称；不得从项目名称截取、扩展或推断这些名称。
...
7. 受保护基础字段（受保护字段）不得删除：`设备名称及数量`、`交付日期`、`交付地点`、`付款方式`、`服务地点`、`服务期限`、`预算/最高限价`、`包号/标段号` 是采购需求正文必须保留的基础信息字段。这些字段的字段壳和字段存在性来自参考模板（不是“旧事实”）。字段值优先级为：`项目基础信息同字段/同语义槽位 > 技术参数项目概述同字段/同语义槽位 > 参考模板同包字段原句`。该优先级不能跨槽位使用，尤其不能用项目名称推断设备名称；只有前两个来源都没有该槽位新值时，模板才能兜底。即使当前项目材料（项目基础信息/技术参数）没有为这些字段提供新值，也必须保留字段行（继承参考模板同包字段行原句或占位表达），**绝不能因为“无新材料支撑”而要求删除**这些字段。字段存在性优先级高于旧事实清理：审核只能要求补回/恢复受保护字段行，不能要求删除。多包场景必须逐包判断字段存在性，不能因为全文某一包出现过字段名就判定其它包也存在。
...
必查规则：
0. **事实原子逐字审计（第一优先级）：**
   - 第一步，按“同包 + 同一设备或服务对象”把【技术参数】拆成来源字段标签、目标语义槽位和原始值。优先同名字段；只有目标模板明显只是给同一事实换字段壳时，才允许标签对齐。例如`维保设备`可对齐设备清单的`设备名称`，`型号`可对齐`规格型号`，`数量：1套`可无损拆成`数量=1`与`单位=套`。拆分字符按原顺序拼回后必须与原值相同。这些只是结构变化，不报 finding。
   - 第二步，项目概述受保护字段按“【项目基础信息】明确同槽位 > 【技术参数】项目概述同槽位 > 【参考内容】同槽位兜底”确定应采用值；无标签项目基础信息不得靠猜测覆盖技术参数明确值。技术/服务/商务正文和设备清单单元格以【技术参数】同槽位原值为准。模板只有在前两个来源都没有该槽位值时才能兜底。项目名称默认不能作为设备/维保设备/服务/采购标的名称的取值来源。
   - 第三步，逐字比较原值和现值。除章节壳、字段标签壳、编号外形、换行/缩进、模板容器、复合值无损拆格、规定的重要性标识字形规范化、评分污染删除和系统来源标记删除外，任何文字变化都必须报 finding。必须覆盖但不限于：(a) 名称、品牌、型号、专有名词增加或删除限定词；(b) 同义替换、简称展开、术语“规范化”、语序润色；(c) 数字与中文数字互换；(d) 数值、单位、范围、比例、频次、期限、地点、条件、否定词、比较符号变化；(e) 把项目名称、文件名、模板旧值或相邻字段拼接进设备名称/服务名称。即使语义看似相同，只要事实值字符不同也要报 finding，并在 evidence 中同时引用应采用的原值和现正文值，fix_hint 要求逐字恢复原值。
1. 先从【技术参数】逐行/逐表格行抽取所有带重要性标识（紧邻编号前后的 `★/▲/△/Δ/*/#/※/●` 等，例如 Symbol 字体抽取出的 `Δ`）的原子指标，形成唯一白名单；再审核【待审核正文】中的这些指标。技术参数中的每一条带标识的指标都必须在待审核正文中保留同类标识和对应参数内容；同类指三角类（`△/Δ/▲…`）或星类（`*/※/★…`），正文输出规范字形 `▲`/`★` 视为正确，不得因源字形是 `△/Δ/*` 而报“符号被改写”。指标归属、核心文字、数值、单位、所在原子条款或表格行必须与技术参数完全一致；除必要编号重排外，不能缺漏、降级为普通条款、换成另一类符号、合并到其它条款或拆散导致符号归属变化。正文中的技术符号（`≥/±/×/Ω/SpO₂/℃` 等计量/科学符号）不是重要性标识，不参与本条（重要性标识白名单）审核；但它们属于参数文字，是否被逐字保留由下面第 14 条【特殊符号保真总则】单独审核，绝不能因为“不是重要性标识”就放过任何符号丢失或改写。
...
6. 基础信息章节格式镜像规则：若参考内容含有“项目概述/项目基本情况/项目概况/总体需求”等基础信息章节，必须按模板动态识别其中出现的【每一个】项目元数据字段行/字段表，不得硬编码只审核交付日期或付款方式，也不得只盯住某一个固定清单里的字段。项目元数据字段指以“字段名：值”形式承载项目级基础事实的静态字段，判定按性质界定，包括但不限于：项目名称/采购名称/服务名称、设备名称及数量、数量/单位、交付地点/服务地点、交付日期/服务期限/合同履约期限、付款方式、预算/最高限价、包号/标段号、项目编号等同类基础信息字段。模板首章里出现的每一个此类关键字段，正文都必须按模板字段壳逐项保留；缺失时必须按模板字段一一报缺并要求补回。多包时必须建立“包号 × 模板字段”的检查表，对每个包单独输出缺失字段 finding。服务范围、维保范围、设备清单、过滤器清单、备品备件、人员配备、资质要求、报价要求、质量保证、执行标准、评分/评审内容等不是项目元数据字段，即使出现在项目概述里也不得当作必须保留的字段壳。
...
14. 特殊符号保真总则（通用，覆盖一切特殊符号）：【技术参数】里出现的所有特殊符号与非常规字符，都必须在【待审核正文】对应条款中如实保留；只要发生丢失、改写成近义文字或 ASCII 近似、或符号挂到错误条款，都必须输出 finding 要求恢复。判定范围按性质界定，包括但不限于：(a) 重要性标识类——紧邻编号前后、用于标注条款重要性的非数字非字母标记（如 `★/▲/△/Δ/☆/◆/◇/*/#/※/●` 等），其有无与归属按上文重要性标识规则审核；条款位置的 `△/Δ→▲`、`*/※→★` 规范字形视为正确，不得报 finding 要求改回源字形；(b) 计量与科学符号类——如 `≥/≤/＜/＞/±/×/÷/·/√/∑/∞/‰/%/℃/℉/°/′/″/Ω/μ/λ/Φ/π` 及上下标 `²/³/₂` 等；(c) 其它非 ASCII、全角、单位、货币或箭头等特殊字符。典型错误：把 `≥` 写成 `>=` 或“大于等于”、把 `℃` 丢成“度”、把 `±` 改成“正负”、漏掉条款位置的星/三角标识、把 `Ω` 写成“欧姆”等，均须报 finding 恢复。`ΔT`、`5*6`、型号中的 `*` 等技术符号不在 (a) 类规范范围内，必须原样保留。
15. 带锚点表审核总则：若【技术参数】中一组连续表格行后紧跟 `[[TABLE:<id>]]`，必须先看【生成风格】。A）评分/评审表：任何生成风格下都删除标题、投影表和锚点。B）template 生成风格：若锚点前表格只有 1 个有效列，或每行只是标题、段落、编号条款、技术参数长句（如 `| 一、设备名称：... |`、`| ▲3.1具备... |`），待审核正文把表内文字展开为普通技术正文并删除 `[[TABLE:<id>]]` 是正确行为；若此时仍保留该锚点，必须输出 finding 要求删除该文本容器表锚点，避免写回层重复插入原表。C）param 生成风格：非评分锚点表必须锚点直通；即使锚点前是一列表格包住整段技术参数，也应保留 `[[TABLE:<id>]]` 独占一行并删除投影表行，不得展开成普通正文。D）真实数据表/附件表/配置清单表：若表格有明确多列字段 schema（如 `序号/名称/数量/备注`、`项目/要求`）且各行是同类记录，待审核正文必须保留该锚点独占一行，并且不得把锚点前投影表行重复转写成普通正文、编号列表、纯文本表格或手绘表格。
16. 锚点邻接正文审核总则：`[[TABLE:<id>]]` 只代表紧邻它之前的一张锚点表，不代表表格前后的附件标题、报价说明、注释、签字盖章、日期、后续章节或其它非表格正文。审核时必须逐段检查【技术参数】中每个非评分锚点前后的非表格行：如果待审核正文把多个锚点集中到文末、连续输出多个 `[[TABLE:...]]` 后结束，或缺失两个锚点之间/锚点之后的真实非表格正文，必须输出 finding 要求按技术参数原位置恢复这些非表格正文；不要要求重画表格，不要恢复评分表。template 生成风格中文本容器表锚点被删除时，不按“缺失锚点”报错。
17. 包头裸机构行检查：若待审核正文在每个包标题前后重复输出单独一行采购人、采购单位、医院、学校、公司等机构名称（例如 `上海市第六人民医院`），且该行不是 `交付地点/服务地点` 等字段值，也不是技术参数条款，必须输出 finding 要求删除该裸行。若包标题保留 `项目技术参数/技术参数/参数要求` 等来源说明后缀，也应要求净化为 `第N包：设备或服务名称`。
18. 技术参数可见编号审核：技术参数正文中的连续参数条款必须在待审核正文中有可见编号。若【技术参数】或抽取上下文表现为连续参数行（如 `字段名：要求`、含 `≥/≤/%/MHz/mm/组/帧/套` 等指标值，或行首带 `★/▲/△/Δ/*/#/※/●`），而待审核正文在技术需求章节中把这些行无编号直出（如 `成像模式：B模式`、`▲产品形态：一体便携式`），必须输出 finding，要求按物理顺序补连续编号；重要性标识保留在对应条款正文前，不得删除或换条款。不要因为源材料的 Word 自动编号在抽取文本中不可见，就放过无编号清单。
...
Few-shots：
输入要点：技术参数含`维保设备：磁共振系统；数量：1套；型号：Ingenia 3.0T`；待审核正文把同一对象放入模板设备清单，分别写成`设备名称=磁共振系统`、`规格型号=Ingenia 3.0T`、`数量=1`、`单位=套`，所有值字符均未改变。
输出：
[]

输入要点：项目基础信息含`项目名称：医用核磁共振系统维保`；技术参数含`维保设备：磁共振系统`；待审核正文的项目名称正确写成`医用核磁共振系统维保`，但设备清单写成`医用核磁共振系统`。
输出：
[{"evidence":"原技术参数的维保设备值为 `磁共振系统`，但待审核正文设备清单写成 `医用核磁共振系统`，擅自增加了 `医用核`；项目名称不能作为设备名称的取值来源。","fix_hint":"只将设备清单中的设备名称恢复为原技术参数逐字值 `磁共振系统`；保留项目名称 `医用核磁共振系统维保` 和其它内容不变。"}]

输入要点：项目基础信息未提供服务期限；技术参数含`服务期限：3年`和`响应时间≤2小时`，待审核正文写成`服务期限：三年`和`响应时间不超过2小时`。
输出：
[{"evidence":"原技术参数字段 `服务期限：3年` 在待审核正文中被改成 `服务期限：三年`，数字写法发生变化。","fix_hint":"将该字段值逐字恢复为 `3年`，保持字段壳和其它内容不变。"},{"evidence":"原技术参数条款 `响应时间≤2小时` 在待审核正文中被同义改写为 `响应时间不超过2小时`。","fix_hint":"将该条款逐字恢复为 `响应时间≤2小时`，保持编号和其它内容不变。"}]
...
输入要点：参考内容每包项目概述均含 `1、设备名称及数量`、`2、交付日期`、`3、交付地点`、`4、付款方式`；待审核正文三包中第1包和第3包项目概述缺少 `付款方式`，第2包有 `付款方式`。
输出：
[{"evidence":"参考内容要求每个包的项目概述都包含 `付款方式：` 字段，但待审核正文第1包项目概述缺少该字段；不能因第2包已出现付款方式就视为第1包已满足。","fix_hint":"在第1包项目概述中按参考模板同包字段顺序补回 `付款方式：` 字段行；当前材料无新值时保留参考模板同包字段原句，保持其它内容不变。"},{"evidence":"参考内容要求每个包的项目概述都包含 `付款方式：` 字段，但待审核正文第3包项目概述缺少该字段；不能因第2包已出现付款方式就视为第3包已满足。","fix_hint":"在第3包项目概述中按参考模板同包字段顺序补回 `付款方式：` 字段行；当前材料无新值时保留参考模板同包字段原句，保持其它内容不变。"}]

输入要点：待审核正文每包开头写成 `上海市第六人民医院\n第1包：病人监护仪项目技术参数\n一、项目概述...`。
输出：
[{"evidence":"待审核正文第1包标题前存在单独机构名称行 `上海市第六人民医院`，该行不是项目概述字段值或技术参数条款；包标题 `第1包：病人监护仪项目技术参数` 还保留了来源说明后缀 `项目技术参数`。","fix_hint":"删除该包标题前的裸机构名称行，并将包标题净化为 `第1包：病人监护仪`；保持项目概述字段和后续技术需求内容不变。"}]

输入要点：参考内容含 `2、付款方式：设备安装验收合格后的三个月内付清全款。`，但项目基础信息和技术参数都没有付款方式；某审核意见以“无新材料支撑的旧事实”为由要求删除该付款方式行。
输出：
[]

输入要点：技术参数含 `1.1、工作温度：≥-20℃`，待审核正文写成 `1、工作温度：大于等于-20度`。
输出：
[{"evidence":"技术参数 `1.1、工作温度：≥-20℃` 中的 `≥` 和 `℃` 在待审核正文 `1、工作温度：大于等于-20度` 中被改写成“大于等于”和“度”，特殊符号丢失。","fix_hint":"按技术参数原样恢复 `≥` 与 `℃`，写成 `工作温度：≥-20℃`，保持其它内容不变。"}]

输入要点：技术参数含 `2.1、测量精度：±0.5%`，待审核正文写成 `2、测量精度：0.5%`。
输出：
[{"evidence":"技术参数 `2.1、测量精度：±0.5%` 中的 `±` 在待审核正文 `2、测量精度：0.5%` 中丢失。","fix_hint":"恢复 `±` 符号，写成 `测量精度：±0.5%`，保持数值与其它内容不变。"}]

输入要点：技术参数因 Word 自动编号抽取丢失而呈现为 `成像模式：B模式\n▲产品形态：一体便携式\n图像旋转：支持360°旋转`，待审核正文也无编号输出这三行。
输出：
[{"evidence":"技术需求中连续参数行 `成像模式：B模式`、`▲产品形态：一体便携式`、`图像旋转：支持360°旋转` 被无编号直出，缺少可见条款编号。","fix_hint":"按物理顺序补成连续编号，例如 `1、成像模式：B模式`、`2、▲产品形态：一体便携式`、`3、图像旋转：支持360°旋转`，保持参数文字和 ▲ 标识不变。"}]

输入要点：技术参数含 `3.1、量程：0~100℃，分辨率0.1℃`，待审核正文写成 `1、量程：0~100℃，分辨率0.1℃`，仅编号按模板重排。
输出：
[]

输入要点：技术参数含 `6. 详细配置清单参数如下：\n| 序号 | 名称 | 数量 |\n| 1 | A | 1 |\n[[TABLE:TP1_1]]`，待审核正文写成 `6、详细配置清单参数如下：\n[[TABLE:TP1_1]]`，没有把投影表行转成可见正文。
输出：
[]

输入要点：技术参数含 `二、技术需求\n| 1 | 设备用途 |\n| --- | --- |\n| 1.1 | 用于监测。 |\n[[TABLE:TP1_1]]`，待审核正文写成 `二、技术需求\n1 设备用途\n1.1 用于监测。\n[[TABLE:TP1_1]]`。
输出：
[{"evidence":"技术参数中 `[[TABLE:TP1_1]]` 前的 `| 1 | 设备用途 |`、`| 1.1 | 用于监测。 |` 是结构化表投影预览，但待审核正文把它重复转写为普通文本 `1 设备用途`、`1.1 用于监测。`，同时保留 `[[TABLE:TP1_1]]`。","fix_hint":"删除该锚点表对应的普通文本/手绘投影行，保留必要章节标题、表前表后非表格正文和独占一行的 `[[TABLE:TP1_1]]`；不要重写其它正文。"}]

输入要点：技术参数含 `（四）人员要求\n| 序号 | 岗位 |\n| --- | --- |\n| 1 | 保洁 |\n[[TABLE:TP1_1]]\n5、附件5：《专项保洁报价单》\n（1）每年一次外墙清洗，投标人需自报单价。\n| 序号 | 服务内容 |\n| --- | --- |\n| 1 | 外墙清洗 |\n[[TABLE:TP1_2]]\n注：超过的按实际发生面积结算。\n投标人授权代表签字或盖章：`，待审核正文在文末只写 `[[TABLE:TP1_1]]\n[[TABLE:TP1_2]]`。
输出：
[{"evidence":"技术参数中 `[[TABLE:TP1_1]]` 与 `[[TABLE:TP1_2]]` 之间存在非表格正文 `5、附件5：《专项保洁报价单》`、`（1）每年一次外墙清洗，投标人需自报单价。`，且 `[[TABLE:TP1_2]]` 后还有 `注：超过的按实际发生面积结算。`、`投标人授权代表签字或盖章：`；待审核正文只在文末连续输出两个表格锚点，缺失这些非表格正文。","fix_hint":"把每个表格锚点放回技术参数原位置，只删除投影表行，恢复锚点之间和锚点之后的附件标题、报价说明、注释及签字盖章行；不要恢复评分表，也不要重画结构化表。"}]
""".strip()
VERIFY_JSON_REPAIR_SYSTEM_PROMPT = (
    "你是 JSON 修复助手。只把输入修复为严格合法的 JSON 数组。"
    "数组每项必须包含非空字符串 evidence 和 fix_hint。禁止新增审核问题，禁止解释。"
    "如果某项表达无问题、符合要求、无需修改，或 fix_hint 是“无/无需修改/保持不变”，删除该项。"
)

# 确定性硬契约提示：[[TABLE:id]] 按生成风格决定直通或展开。
TABLE_PLACEHOLDER_CONTRACT_PROMPT = (
    "【结构化表占位符硬契约】技术参数中的 `[[TABLE:<id>]]` 是结构化表的内部写回入口，属于运行时硬契约；审核时必须先看【生成风格】。\n"
    "- 经写回层处理后，`[[TABLE:id]]` **不应作为可见行**出现在最终 Word 里：写回层会按 id 把它替换成真实表格，或在无法恢复时静默丢弃。\n"
    "- **param 生成风格**：非评分 `[[TABLE:id]]` 占位符是正确且被要求的行为。即使锚点前是一列表格包住整段技术参数，也应保留锚点并删除投影表行，由写回层直接恢复技术参数表；不要因为正文中出现占位符就输出 finding 要求删除它。\n"
    "- **template 生成风格**：一列表格技术参数属于文本容器表时，可以展开为普通正文并删除对应 `[[TABLE:id]]`；不要为这种缺失锚点报错。若已经展开表内文字却仍保留同一 `[[TABLE:id]]`，必须输出 finding 要求删除该文本容器锚点。\n"
    "- **真实数据表/附件表/配置清单表**：任何生成风格下都应保留 `[[TABLE:id]]` 并删除投影表行，不要手绘或散文化替代。\n"
    "- **必须检查锚点表是否被重复散文化**：如果【技术参数】中某个应保留的 `[[TABLE:id]]` 前紧邻的是表格投影，而【待审核正文】除了保留 `[[TABLE:id]]` 外，还把同一表格投影内容转写成普通文本、编号列表、Markdown/纯文本表格或手绘表格，必须输出 finding 要求删除这些重复投影内容，只保留锚点。\n"
    "- **必须检查锚点邻接正文是否丢失或挪位**：`[[TABLE:id]]` 只代表紧邻它之前的一张表格，不代表表格标题、附件标题、说明、注释、签字盖章、日期或后续章节。若待审核正文把多个占位符集中到文末，或省略两个占位符之间/占位符之后的非表格正文，必须输出 finding 要求恢复这些非表格正文并把锚点放回原位置。\n"
    "- **不要**为缺失的 `[[TABLE:id]]` 单独输出 finding：如果该结构化表所在章节和表格内容已经整体删除，或者只剩其它可靠非表格正文，占位符恢复与否由写回层处理，不进入审核。\n"
    "- 但在 param 生成风格下，如果该结构化表所在采购需求正文仍被保留，且待审核正文用 Markdown 表格、手绘表格、普通列表或散文参数替代了 `[[TABLE:id]]`，这不是“缺失占位符”，而是表格投影被错误可见化，必须输出 finding，要求删除可见投影内容并恢复独占一行的 `[[TABLE:id]]`。\n"
    "- 无锚点普通表仍按技术参数表格 schema 审核；有锚点表按生成风格审核锚点直通或文本展开、投影不重复、评分污染删除。\n"
    "- 模型只需正常输出该表所在章节的其它非表格文本参数；为缺失的表补“须提供详细…”之类占位句是被禁止的兜底行为，可作为 finding 报出。\n"
)
...
def _render_verify_user_prompt(
    *,
    ...
    return (
        ...
        "输出前过滤门：只有存在可定位正文错误、必须修改、且能写出具体编辑动作的候选，才允许进入 JSON 数组。"
        "凡结论是“符合要求、无问题、实质一致、无需修改”，或 fix_hint 只能写“无/无需修改/保持不变”的候选，必须删除；"
        "如果没有剩余候选，只输出 []。不要把检查过程或通过项写进 evidence。\n\n"
        "审核时请严格区分四类输入：\n"
        "1. 【生成风格】决定外层章节壳和基础信息字段壳的审核方式；template 和 param 都不能降低【技术参数】对参数本体、表格 schema、物理顺序和重要性标识（★/▲/△/Δ/*/#/※/● 等）的硬审核优先级，不能用参考内容审核参数本体。\n"
        "2. 【项目基础信息】只提供明确标注或可无歧义定位的同一项目级语义槽位；无字段标签的分隔片段只能在技术参数没有该槽位值时补充项目级字段，不得靠猜测覆盖技术参数明确值。项目名称默认不是设备名称、维保设备、服务名称或采购标的名称，禁止跨槽位推断。\n"
        ...
        "重点检查：\n"
        "- 【事实原子逐字审计，必须先做】先建立包/设备或服务对象/来源字段标签/目标语义槽位/原始字段值账本，再逐字段比对正文。允许模板给同一槽位换字段标签，允许把`维保设备`对齐到设备清单`设备名称`，也允许把`数量：1套`无损拆成`数量=1、单位=套`；拼回原值一致时不报。项目概述受保护字段先按同槽位优先级确定应采用值，技术/服务/商务正文和设备清单以技术参数同槽位为原值。除章节壳、字段标签壳、编号外形、换行/缩进、模板容器、无损拆格、规定的标识规范化和污染删除外，名称限定词、同义词、简称、数字写法、数值、单位、范围、比例、频次、期限、地点、条件、否定词或比较符号的任何增删改都必须输出 finding；语义相近也不能放过。evidence 同时引用原值和现值，fix_hint 要求恢复原值。项目名称不得用于补全设备名称。\n"
        "- 【编号隔离硬规则】先从原始条款和待审核正文每个原子行首剥离纯结构编号（`1、`、`2.1、`、`（1）`、`一、`等）再比较事实值。仅新增、删除、替换、重排或恢复编号时，必须视为结构一致，不得生成 finding；例如 `通道反转分析`→`4、通道反转分析`、`无线连接技术……`→`1、无线连接技术……` 都应省略。只有连续参数条款完全没有可见编号时，才按编号完整性规则报 finding。编号后的 `★/▲` 与正文文字另行核对。\n"
        "- 按审核流程先拆分【技术参数】原子条款，再建立重要性标识白名单；不要直接按参考内容槽位或旧序号匹配。\n"
        "- 先从【技术参数】逐行/逐表格行抽取所有带重要性标识（★/▲/△/Δ 等）的原子指标，形成唯一白名单；待审核正文中的每一个重要性标识指标都必须命中该白名单。正文中的技术符号（≥/±/×/Ω/SpO₂/℃ 等）不是重要性标识，不参与重要性标识白名单审核，但仍要按下一条做符号保真审核。\n"
        "- 【特殊符号保真】技术参数里出现的所有特殊符号/非常规字符——重要性标识（★/▲/△/Δ/※/● 等）、计量与科学符号（≥/≤/±/×/÷/√/℃/℉/°/Ω/μ/π/‰/% 及上下标 ²/³/₂ 等）、以及其它全角/单位/货币/箭头符号——都必须在正文对应条款中逐字保留；凡丢失、改写成近义文字或 ASCII 近似（如 ≥→>=、≥→“大于等于”、℃→“度”、±→“正负”）、或挂到错误条款的，都要输出 finding 要求按技术参数恢复。技术符号只审核“是否逐字保留”，不据此增删 ★/▲。\n"
        ...
        "- 项目概述关键字段完整性：模板的项目概述/项目基本情况里有哪些项目元数据关键字段（项目名称、设备名称及数量、数量/单位、交付地点/服务地点、交付日期/服务期限、付款方式、预算/最高限价、包号/标段号、项目编号等同类基础字段），正文就必须有哪些；任何一个模板已有的关键字段在正文中缺失，都要逐一输出 finding，要求按模板项目概述的字段名、字段顺序和格式壳一一补回（缺值时保留模板原句或占位表达），不得遗漏任何一个模板关键字段，也不得编造新值。多包场景必须逐包检查，不能因为全文某一包出现过字段名就放过其它包。\n"
        ...
        "- 受保护字段不得删除：`设备名称及数量/交付日期/交付地点/付款方式/服务地点/服务期限/预算/最高限价/包号/标段号` 的字段壳来自参考模板（不是旧事实），字段值优先级仅限事实账本确认的同一字段/同一语义槽位：项目基础信息同槽位 > 技术参数项目概述同槽位 > 参考模板同包字段原句。只有前两个来源都缺值时模板才能兜底，即缺值继承模板同包字段原句或占位表达。即使当前材料无新值，也必须保留字段行，不得因为“无新材料支撑”要求删除；字段存在性优先级高于旧事实清理，删除建议无效。多包时逐包判断字段存在性。\n"
        "- 包头裸机构行和标题后缀：待审核正文不得在每包开头重复输出单独的采购人/采购单位/医院/学校/公司名称行；这些裸机构行不是 `交付地点/服务地点` 字段值时必须删除。包标题中的 `项目技术参数/技术参数/参数要求` 等来源说明后缀也应删除，净化为 `第N包：设备或服务名称`。\n"
        ...
        "- 技术参数清单必须有可见编号：如果待审核正文在技术需求章节连续输出 `成像模式：...`、`▲产品形态：...`、`图像旋转：...` 这类参数行且完全没有 `1、2、3、...` 或参考内容对应样式编号，必须输出 finding，要求按物理顺序补连续编号，并保留 ★/▲ 等标识在原条款；如果已经有可见编号，即使编号值或标点与原材料不同，也不得仅因编号差异输出 finding。\n"
        "- 结构化表必须按生成风格审核：若【技术参数】中连续 `|...|` 表格行后紧跟 `[[TABLE:<id>]]`，param 生成风格必须保留锚点独占一行并删除投影表行；template 生成风格遇到一列表格技术参数时，可以展开表内文字并删除对应锚点。\n"
        "- 真实数据表/附件表/配置清单表在任何生成风格下都保留锚点；若待审核正文把同一投影行改成编号列表、Markdown/纯文本表格或手绘表格，同时保留锚点，必须输出 finding，要求删除这些重复投影内容，保留必要章节标题、表前表后非表格正文和 `[[TABLE:<id>]]`。\n"
        "- 结构化表占位符不得吞掉正文：逐段检查【技术参数】中每个非评分锚点前后是否有附件标题、报价说明、注释、签字盖章、日期或后续章节等非表格正文；若这些正文在【待审核正文】缺失，或多个 `[[TABLE:...]]` 被集中移到文末连续输出，必须输出 finding 要求恢复这些非表格正文并把锚点放回原位置。template 文本容器表锚点被删除时，不按缺失锚点报错。\n"
        ...
        + TABLE_PLACEHOLDER_CONTRACT_PROMPT
        + "\n"
        "受保护字段硬契约：受保护基础字段（设备名称及数量/交付日期/交付地点/付款方式/服务地点/服务期限/预算/最高限价/包号/标段号）不得删除；字段值只按事实账本确认的同一字段/同一语义槽位取值，优先级为 项目基础信息同槽位 > 技术参数项目概述同槽位 > 参考模板同包字段原句；只有前两个来源都缺值时模板才能兜底，删除建议无效，且不得跨槽位推断。\n"
        "只返回需要修复的问题；不需要修改的问题不要出现在 JSON 数组里。"
        "如果比对结论是“实质一致、无问题、无需修改”，必须输出 []，"
        "不要把一致性说明写成 evidence。"
        "fix_hint 禁止写“无/无需修改/保持不变”；若只能写这些内容，说明该候选不是问题，必须省略。\n\n"
