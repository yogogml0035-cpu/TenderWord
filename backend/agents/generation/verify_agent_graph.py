...
15. 结构化表审核总则：若【技术参数】中一组连续表格行后紧跟 `[[TABLE:<id>]]`，这组表格行只是结构化表投影预览，`[[TABLE:<id>]]` 才是真实表写回锚点。待审核正文必须保留该锚点独占一行，并且不得把锚点前投影表行重复转写成普通正文、编号列表、纯文本表格或手绘表格。典型错误：技术参数含 `| 1 | 设备用途 | ... [[TABLE:TP1_1]]`，待审核正文写成 `1 设备用途`、`1.1 用于...`、`2 配置要求`，同时又保留 `[[TABLE:TP1_1]]`；这种“锚点 + 散文化投影表”必须输出 finding，要求删除散文化投影行，只保留章节标题和 `[[TABLE:TP1_1]]`。若该锚点表属于评分污染，则标题、投影表和锚点都应删除。
...
输出：
[{"evidence":"技术参数中 `[[TABLE:TP1_1]]` 前的 `| 1 | 设备用途 |`、`| 1.1 | 用于监测。 |` 是结构化表投影预览，但待审核正文把它重复转写为普通文本 `1 设备用途`、`1.1 用于监测。`，同时保留 `[[TABLE:TP1_1]]`。","fix_hint":"删除该锚点表对应的普通文本/手绘投影行，只保留必要章节标题和独占一行的 `[[TABLE:TP1_1]]`；不要重写其它正文。"}]
""".strip()
...
def _render_verify_user_prompt(
    *,
    ...
    return (
        ...
        "- 结构化表必须按锚点审核：若【技术参数】中连续 `|...|` 表格行后紧跟 `[[TABLE:<id>]]`，这些表格行是投影预览，不是正文条款；【待审核正文】只允许保留该锚点独占一行。若待审核正文把同一投影行改成 `1 设备用途`、`1.1 ...`、编号列表、Markdown/纯文本表格或手绘表格，必须输出 finding，要求删除这些重复投影内容，只保留必要章节标题和 `[[TABLE:<id>]]`。\n"
