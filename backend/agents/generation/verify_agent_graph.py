...
3. 【参考内容】是基础模板格式真源：一级章节名称与顺序、项目概述/项目基本情况/项目概况/总体需求/商务/售后等非参数结构、字段标签、字段顺序、编号标点、冒号形式、单行/多行字段容器、表格/纯文本容器、占位符/方括号样式和语气模具都要被审核；但它不是旧事实真源，不得要求沿用旧设备名、旧日期、旧金额、旧参数或旧保修期。
4. template 生成风格要求高保真镜像参考内容的格式壳，再用项目基础信息和技术参数替换事实内容。
5. param 生成风格同样要求基础模板格式一致；差异只在参数章节内部按参数优先生成提示词执行技术/服务条款接管、旧标题粉碎、无源旧事实删除、旧表壳清洗和连续重编号。
...
5. 所有生成风格下，待审核正文都必须继承参考内容的基础非事实格式：一级章节名称与顺序、项目概述/项目基本情况/项目概况/总体需求/商务条款等非参数结构、字段标签、字段顺序、编号标点、冒号形式和表格/纯文本容器。参考内容里的旧事实必须被项目基础信息或技术参数替换，但这些格式壳不能无故丢失、拆散或改成另一套章节。
6. 基础信息章节格式镜像规则：若参考内容含有“项目概述/项目基本情况/项目概况/总体需求”等基础信息章节，必须按模板动态识别该章节内的字段行/字段表，不得硬编码只审核交付日期或付款方式；模板里出现哪些字段，就审核哪些字段。
7. 基础信息章节必须按模板原格式壳生成：章节名、章节编号样式、字段标签、字段顺序、冒号形式、字段值与字段名同一行/同一单元格关系、单行/多行容器、表格/纯文本容器、占位符/方括号样式都要跟随模板。禁止把字段改写成散文句、合并成一段、拆到其它章节、换成另一套编号或从表格改成纯文本/从纯文本改成表格。
8. 区分“旧事实”和“可继承的格式壳”：旧设备名、旧日期、旧金额、旧参数值不能继承；字段名、编号、冒号、固定提示语、占位符、方括号和表格/文本容器可以继承。若模板有字段但项目基础信息和技术参数没有给出新值，finding 必须要求恢复该字段行/字段表并保留模板占位符或固定表达，不允许建议删除字段、拆章或编造值。
9. template 生成风格下，技术参数应注入参考内容中的第一个核心技术章节；仅当技术参数已经接管旧技术章节时，才允许删除参考内容后续冗余技术类章节并顺延大标题序号。不能因为技术参数完整就丢掉参考内容中的非技术基础结构。
10. param 生成风格下，参数章节内部必须按参数优先生成提示词审核：项目元数据只覆盖参考内容已有语义槽位；技术/服务条款由技术参数按物理顺序接管；旧模板中无新材料支撑的引导段、设备专属子标题、旧章节主体、旧表格列名/行数和旧商务事实必须删除；删除后编号必须从当前有效条目连续重排。
11. 仅编号样式、章节外壳、标点或表格形态跟技术参数原文不同，不算问题；这些应按参考内容基础格式和当前生成风格判断。
...
def _render_verify_user_prompt(
    *,
    ...
    return (
        ...
        "审核时请严格区分四类输入：\n"
        "1. 【生成风格】决定参数章节内部审核方式；template 和 param 都必须校验基础模板格式一致。\n"
        "2. 【项目基础信息】提供项目名称、数量、交付、付款等基础事实。\n"
        "3. 【参考内容】不是旧事实真源；它是章节、字段、字段顺序、编号、冒号形式、单行/多行字段容器、表格/纯文本容器、占位符/方括号样式和语气的基础格式真源。\n"
        "4. 【技术参数（原材料，事实真源）】提供必须进入正文的实质参数、★/▲指标和包件数量。\n\n"
        "重点检查：\n"
        "- 技术参数中出现的 ★、▲ 指标是否在正文中逐项保留同类符号和对应参数内容。\n"
        "- 正文是否额外增加了技术参数没有的 ★、▲ 指标。\n"
        "- 技术参数明显是多个包件/标段/采购包/独立设备组时，正文是否只生成了其中一个。\n"
        "- 正文是否用参考内容旧事实替换了技术参数或项目基础信息中的新事实。\n"
        "- 基础信息章节按模板动态识别，覆盖项目概述/项目基本情况/项目概况/总体需求；逐项比对该章节的模板字段列表、字段顺序和字段行/字段表容器。\n"
        "- 基础信息字段行/字段表格式壳必须继承模板：章节名、编号样式、字段标签、冒号形式、单行/多行容器、表格/纯文本容器、占位符/方括号样式都要跟随参考内容。\n"
        "- 检查字段值是否仍与字段名保持模板中的同一行或同一单元格关系；不得把字段改写成段落、合并成散文句、拆到其他章节或换容器。\n"
        "- 模板有字段但当前项目材料无新事实时，应保留模板字段壳和占位/固定表达；不得建议删除字段或编造值。`项目预算` 等字段只有在模板基础信息章节本来存在时才纳入该章节审核。\n"
        "- 所有生成风格下，正文是否丢失或拆散参考内容的项目概述、项目基本情况、项目概况、总体需求、商务/售后等基础模板结构。\n"
        "- param 模式下，参数章节是否符合参数优先生成提示词：技术/服务条款接管、旧标题粉碎、无源旧事实删除、旧表壳清洗和连续重编号。\n"
        "- 不要因为正文编号、章节外壳、表格形态与技术参数原文不同而报错；这些应按参考内容和生成风格判断。\n\n"
