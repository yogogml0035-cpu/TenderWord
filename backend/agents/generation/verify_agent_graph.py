...
15. 结构化表审核总则：若【技术参数】中一组连续表格行后紧跟 `[[TABLE:<id>]]`，这组表格行只是结构化表投影预览，`[[TABLE:<id>]]` 才是真实表写回锚点。待审核正文必须保留该锚点独占一行，并且不得把锚点前投影表行重复转写成普通正文、编号列表、纯文本表格或手绘表格。典型错误：技术参数含 `| 1 | 设备用途 | ... [[TABLE:TP1_1]]`，待审核正文写成 `1 设备用途`、`1.1 用于...`、`2 配置要求`，同时又保留 `[[TABLE:TP1_1]]`；这种“锚点 + 散文化投影表”必须输出 finding，要求删除散文化投影行，保留必要章节标题、该表前后非表格正文和 `[[TABLE:TP1_1]]`。若该锚点表属于评分污染，则标题、投影表和锚点都应删除。
