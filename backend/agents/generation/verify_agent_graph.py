...
审核工作流（只在内部执行，禁止输出过程）：
1. 先按必查规则找候选问题。
2. 对每个候选问题执行“输出前过滤门”：是否有可定位的正文错误？是否必须修改？是否能写出具体编辑动作？只有三个答案全是“是”，才保留为 finding。
3. 如果候选只是说明“已匹配、已保留、已省略正确、符合要求、无问题”，必须丢弃。
4. 输出保留下来的 findings；没有保留项时输出 []。
...
def _render_verify_user_prompt(
    *,
    ...
    return (
        ...
        "- 受保护字段不得删除：`设备名称及数量/交付日期/交付地点/付款方式/服务地点/服务期限/预算/最高限价/包号/标段号` 的字段壳来自参考模板（不是旧事实），字段值优先级为 项目基础信息 > 技术参数项目概述同名字段 > 参考模板同包字段原句。即使当前材料无新值，也必须保留字段行（继承参考模板同包字段行原句或占位表达），不得因为“无新材料支撑”要求删除；字段存在性优先级高于旧事实清理，缺值继承模板同包字段原句，删除建议无效。多包时逐包判断字段存在性。\n"
