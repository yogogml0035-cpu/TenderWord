...
3. 【参考内容】是基础模板格式真源：一级章节名称与顺序、项目概述/项目基本情况/总体需求/商务/售后等非参数结构、字段标签、编号标点、表格/纯文本容器和语气模具都要被审核；但它不是旧事实真源，不得要求沿用旧设备名、旧参数或旧保修期。
...
5. 所有生成风格下，待审核正文都必须继承参考内容的基础非事实格式：一级章节名称与顺序、项目概述/总体需求/商务条款等非参数结构、字段标签、编号标点和表格/纯文本容器。参考内容里的旧事实必须被项目基础信息或技术参数替换，但这些格式壳不能无故丢失、拆散或改成另一套章节。
6. 所有生成风格下，若参考内容含有“项目概述/项目基本情况/总体需求”等基础信息章节及其字段（如设备名称及数量、交付日期、付款方式），待审核正文必须保留等价章节和字段位置，并用当前项目事实替换字段值；缺章、缺字段或把字段拆到无关章节时必须报错。
7. template 生成风格下，技术参数应注入参考内容中的第一个核心技术章节；仅当技术参数已经接管旧技术章节时，才允许删除参考内容后续冗余技术类章节并顺延大标题序号。不能因为技术参数完整就丢掉参考内容中的非技术基础结构。
8. param 生成风格下，参数章节内部必须按参数优先生成提示词审核：项目元数据只覆盖参考内容已有语义槽位；技术/服务条款由技术参数按物理顺序接管；旧模板中无新材料支撑的引导段、设备专属子标题、旧章节主体、旧表格列名/行数和旧商务事实必须删除；删除后编号必须从当前有效条目连续重排。
9. 仅编号样式、章节外壳、标点或表格形态跟技术参数原文不同，不算问题；这些应按参考内容基础格式和当前生成风格判断。
...
输入要点：生成风格为 template 或 param，参考内容含 `一、项目概述\n1、设备名称及数量：球管/壹个\n2、交付日期：合同签订后两个月内交货\n3、付款方式：设备安装验收合格后的三个月内付清全款。`，待审核正文直接从 `一、技术参数` 开始，并把交付日期和付款方式拆成独立大章。
输出：
[{"evidence":"参考内容包含 `一、项目概述` 以及 `设备名称及数量`、`交付日期`、`付款方式` 字段，但待审核正文缺少该项目概述结构，并将交付日期、付款方式拆成独立大章。","fix_hint":"按参考内容恢复项目概述章节及其字段位置，用当前项目事实替换字段值；保持已生成的技术参数内容不变。"}]
...
def _render_verify_user_prompt(
    *,
    ...
    return (
        ...
        "3. 【参考内容】不是旧事实真源；它是章节、字段、编号、表格/纯文本容器和语气的基础格式真源。\n"
        ...
        "- 所有生成风格下，正文是否丢失或拆散参考内容的项目概述、总体需求、商务/售后等基础模板结构。\n"
