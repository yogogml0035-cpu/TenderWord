...
3. 【参考内容】只是受限的基础外层格式线索：只用于一级章节名称与顺序、项目概述/项目基本情况/项目概况/总体需求等基础信息章节的字段标签、字段顺序、编号标点、冒号形式、单行/多行字段容器、表格/纯文本容器、占位符/方括号样式和语气模具；它不得作为技术/服务/商务/售后参数正文、表格列名、表格行数、条款顺序、★/▲符号或参数格式的审核依据。
...
8. 区分“旧事实”和“可继承的格式壳”：旧设备名、旧日期、旧金额、旧参数值、旧服务范围、旧商务/售后条款、旧 ★/▲ 和旧表格 schema 不能继承；基础信息字段名、编号、冒号、固定提示语、占位符、方括号和字段容器可以继承。若基础信息章节模板有字段但项目基础信息和技术参数没有给出新值，finding 必须要求恢复该字段行/字段表并保留模板占位符或固定表达，不允许建议删除字段、拆章或编造值。
...
def _render_verify_user_prompt(
    *,
    ...
    return (
        ...
        "3. 【参考内容】不是旧事实真源，也不是“提供的参数”；它只提供一级章节壳、基础信息章节字段壳、编号标点、冒号、占位符和通用排版线索，不能提供技术/服务/商务/售后参数正文、表格 schema、条款顺序或 ★/▲。\n"
        ...
        "- 所有生成风格下，正文是否丢失或拆散参考内容的项目概述、项目基本情况、项目概况、总体需求等基础信息结构；不得把参考内容中的旧技术/服务/商务/售后章节当作必须恢复的基础结构。\n"
