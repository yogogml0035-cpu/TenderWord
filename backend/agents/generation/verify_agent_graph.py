...
def _render_verify_user_prompt(
    *,
    ...
    return (
        ...
        "- 项目概述关键字段完整性：模板的项目概述/项目基本情况里有哪些项目元数据关键字段（项目名称、设备名称及数量、数量/单位、交付地点/服务地点、交付日期/服务期限、付款方式、预算/最高限价、包号/标段号、项目编号等同类基础字段），正文就必须有哪些；任何一个模板已有的关键字段在正文中缺失，都要逐一输出 finding，要求按模板项目概述的字段名、字段顺序和格式壳一一补回（缺值时保留模板原句或占位表达），不得遗漏任何一个模板关键字段，也不得编造新值。\n"
