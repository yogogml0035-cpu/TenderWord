...
必查规则：
1. 先从【技术参数】逐行/逐表格行抽取所有带重要性标识（紧邻编号前后的 `★/▲/△/Δ/*/#/※/●` 等，例如 Symbol 字体抽取出的 `Δ`）的原子指标，形成唯一白名单；再审核【待审核正文】中的这些指标。技术参数中的每一条带标识的指标都必须在待审核正文中保留同类符号和对应参数内容，符号类型、指标归属、核心文字、数值、单位、所在原子条款或表格行必须与技术参数完全一致；除必要编号重排外，不能缺漏、降级为普通条款、换成另一种符号、合并到其它条款或拆散导致符号归属变化。正文中的技术符号（`≥/±/×/Ω/SpO₂/℃` 等计量/科学符号）不是重要性标识，不参与本条（重要性标识白名单）审核；但它们属于参数文字，是否被逐字保留由下面第 14 条【特殊符号保真总则】单独审核，绝不能因为“不是重要性标识”就放过任何符号丢失或改写。
...
14. 特殊符号保真总则（通用，覆盖一切特殊符号）：【技术参数】里出现的所有特殊符号与非常规字符，无论是否属于重要性标识，都必须在【待审核正文】对应条款中逐字保留；只要发生丢失、改写成近义文字或 ASCII 近似、或符号挂到错误条款，都必须输出 finding 要求按技术参数原样恢复。判定范围按性质界定，包括但不限于：(a) 重要性标识类——紧邻编号前后、用于标注条款重要性的非数字非字母标记（如 `★/▲/△/Δ/☆/◆/◇/*/#/※/●` 等），其有无与归属按上文重要性标识规则审核；(b) 计量与科学符号类——如 `≥/≤/＜/＞/±/×/÷/·/√/∑/∞/‰/%/℃/℉/°/′/″/Ω/μ/λ/Φ/π` 及上下标 `²/³/₂` 等；(c) 其它非 ASCII、全角、单位、货币或箭头等特殊字符。典型错误：把 `≥` 写成 `>=` 或“大于等于”、把 `℃` 丢成“度”、把 `±` 改成“正负”、漏掉 `※`、把 `Ω` 写成“欧姆”等，均须报 finding 恢复。本条只审核“符号是否如实保留”，不改变 (a) 类重要性标识是否成立的判定（后者仍只看技术参数原子条款行首是否自带该标识，编号本身可按规则重排）。
