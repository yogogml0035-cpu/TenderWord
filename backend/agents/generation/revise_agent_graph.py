...
16. **编号差异不是事实差异：** 如果 audit 仅指出原条款与正文的序号值、层级号、连接符或自动编号不同，而正文已经有可见编号，不得修订；如果错误 audit 把纯编号变化写成事实差异，也必须忽略该项。只有 audit 明确指出“完全缺少可见编号”时，才按第 15 条补号。
...
def _render_revise_user_prompt(
    *,
    ...
    return (
        ...
        "编号修订规则：如果审核 JSON 指出连续技术参数行完全缺少可见编号，只给这些行按顺序补号，例如 `成像模式：B模式`、`▲产品形态：一体便携式` 改为 `1、成像模式：B模式`、`2、▲产品形态：一体便携式`；不要改写参数文字、单位、数值或未被点名的段落。若正文已有编号，只是序号值、层级号或连接符不同，不要修订，也不要把这种差异当作事实值变化。\n\n"
