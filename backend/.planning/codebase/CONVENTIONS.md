...
**范围：** `backend/` 源码、`backend/tests/`、`backend/requirements.txt`、`backend/.env.example`、`docs/backend.md`、`docs/interfaces-runtime.md`、`docs/knowledge-validation.md`、既有 `backend/.planning/codebase/` 事实文档。`backend/.env` 文件存在，但不得读取或引用内容。

**关键事实来源：**
- 应用入口与 API：`backend/main.py`、`backend/api/generate.py`、`backend/api/agent.py`、`backend/api/tasks.py`、`backend/api/tender.py`、`backend/api/template_candidates.py`、`backend/api/comment_supplement.py`、`backend/api/stream.py`、`backend/api/download.py`、`backend/api/upload.py`、`backend/api/conversations.py`
- 请求、任务与状态模型：`backend/models/generate.py`、`backend/models/agent_run.py`、`backend/models/task.py`、`backend/models/tender.py`、`backend/models/sse.py`、`backend/models/common.py`、`backend/states/base_state.py`、`backend/states/skill_state.py`
- 任务与 Graph 编排：`backend/services/document_service.py`、`backend/services/agent_run_service.py`、`backend/services/task_service.py`、`backend/graphs/base_graph.py`、`backend/graphs/skill_graph.py`、`backend/skills/rewrite/scripts/runtime.py`
...
- 招标类型 graph 使用 `<form_type>_tender_graph.py`，例如 `backend/graphs/gngk_hw_cz_tender_graph.py`、`backend/graphs/gngk_fw_cz_tender_graph.py`。
...
- FastAPI endpoint 以资源动作命名，例如 `create_generate_task()`、`stream_agent_run()`、`get_tender_data()`、`select_template_candidate()`、`create_comment_supplement_task()`。
...
- 模块级常量使用 `UPPER_SNAKE_CASE`，例如 `TASK_KIND_TO_LLM_NODE`、`TRACKED_PROGRESS_NODES`、`UPLOADED_REWRITE_SOURCE`、`REWRITE_NODE_NAMES`、`TASK_AUDIT_STAGES`、`SUPPORTED_PURCHASE_METHODS`。
- 协议字符串保持稳定原文，例如 `xjcg_tender`、`gngk_hw_cz_tender`、`agent_step`、`uploaded_file`、`comment_supplement`。

**类型：**
- Pydantic model、Graph class、service class 使用 PascalCase，例如 `GenerateRequest`、`AgentRunStreamRequest`、`StandardTenderWorkflowGraph`、`DocumentService`、`TenderResponse`、`ErrorResponse`。
- Enum class 使用 PascalCase，成员使用 `UPPER_SNAKE_CASE`，值使用跨端协议字符串；参考 `backend/models/generate.py`（`FormType`、`LLMModel`、`GenerationStyle`、`GenerationMode`、`CommentGenerationMode`、`StyleWritebackMode`）和 `backend/models/task.py`（`TaskStatus`、`TaskKind`）。
...
4. `backend.*` 绝对导入。
5. `TYPE_CHECKING` 下的类型导入或函数内延迟导入（如 `openai`、graph 类、`backend.config.settings.settings`），用于避免循环依赖。
...
- 典型示例：`create_comment_supplement_task()` 只调用 `get_document_service().create_comment_supplement_task()`，失败时把 service 返回的 `error` 码包装为 `HTTPException.detail`。
...
- 任务内部状态到公共 API 模型的转换集中在 `backend/services/task_service.py`。
...
**Prompt/Agent/Retrieval 层（prompt 无副作用）：**
...
- 初次生成 prompt 统一走 `backend/prompts/generate_prompt.py` 的 `render_generate_prompt()`，返回 `RenderedPrompt(system_prompt, user_prompt)`。

## 请求与任务模型（Pydantic）

- API shape 真源放在 `backend/models/`；FastAPI endpoint 只引用 model，不在 route 内临时拼复杂 DTO。新增或修改字段必须同步前端类型、API client 和相关测试。
- 通用错误/成功 envelope：`backend/models/common.py` 的 `ErrorResponse`（`success=False` + `error` dict + `timestamp`）与 `SuccessResponse`（`success=True` + `message` + `timestamp`）。
...
- `GET /api/tender/{tender_no}` 使用 `TenderResponse`，由 `backend/api/tender.py` 调用 `backend/util/common_util/fetch_tender_data.py`；不支持的采购方式返回非阻断 `warning`，不抛异常。
...
- `backend/services/document_service.py::_init_graph_registry()` 注册当前支持的 graph class：
  - 招标生成：`XjcgTenderGraph`、`GngkHwZcTenderGraph`、`GngkHwCzTenderGraph`、`GngkFwZcTenderGraph`、`GngkFwCzTenderGraph`、`GjgkTenderGraph`（键与 `FormType` 协议值对齐，例如 `gngk_fw_cz_tender`）。
  - 任务 graph 类：`RewriteSkillGraph`、`CommentSupplementGraph`（分别挂在 `REWRITE_SKILL_GRAPH_CLASS`、`COMMENT_SUPPLEMENT_GRAPH_CLASS`，不混入 `FormType` 键）。
- rewrite 默认锚点表 `REWRITE_DEFAULT_ANCHORS` 在 `backend/services/document_service.py`，按 form type 前缀分派 before/after 文本。
...
- `backend/agents/generation/content_sanitizer.py` 的 `sanitize_generated_content()` 是 LLM 生成正文的统一清洗入口：剥离 AI 客套语/结语、Markdown 代码围栏与装饰符、填充性占位句，折叠空行并 trim；**保留** `[[TABLE:<id>]]` 占位符行、技术符号、重要性标识。`looks_like_procurement_content()` 用于判定清洗后是否仍含真实采购内容，纯噪声时返回空。测试在 `backend/tests/agents/test_content_sanitizer.py`。
...
- `content_agent` 自主生成走 `draft → audit → revision → final` 阶段，workspace 文件按轮次成对落盘；阶段与轮次经 `agent_step` 暴露给前端（`ContentAgentStepData.phase` 取 `draft`/`audit`/`revision`/`final`），改阶段或轮次结构时同步 `backend/models/sse.py`、写回 node、service 和前端过程卡。
- 审核轮次上限固定为 `MAX_REVISION_ROUNDS = 3`（`backend/agents/generation/workspace.py`）；越界必须抛 `GenerationAgentProtocolError`，不得增加轮次上限旁路。
- 审核结果序列校验在 `backend/agents/generation/content_agents.py::_validate_audit_revision_sequence()`：有非空审核 finding 的轮次必须对应存在修订产物。
- LLM 审核 JSON 经 `backend/agents/generation/json_utils.py` 统一净化：`parse_audit_findings()` / `filter_noop_audit_findings()` / `coerce_audit_findings()`。
- 受保护字段守卫 `backend/agents/generation/protected_field_guard.py::sanitize_protected_field_findings()` 是 LLM 审核 JSON 解析后的确定性护栏；direct_replace 类型（`gngk_hw_cz`/`gjgk`）无受保护字段 profile，守卫原样返回。
- 多包采购需求必须逐包判断受保护字段存在性；守卫与 prompt 契约双写，改规则时同步 `verify_agent_graph.py` 与 `backend/tests/agents/test_generation_content_agent.py`。

## 技术参数来源块拼接契约

- `backend/nodes/common_word_nodes/extract_tender_params.py` 把多份技术参数文件按上传顺序独立成来源块，固定格式 `第{中文数字}份技术参数文件名称为：{original_name 或文件名}\n内容：\n{抽取文本}`。
- 来源名优先取 `tender_param_files[].original_name`，缺失时回退到后端保存文件名；`tender_param_files` 优先于旧 `tender_param_paths`。
- 多文件结构化抽取按 `file_index` 分配 `table_id` 前缀（`TP1_`/`TP2_`…），对应 `tender_param_table_models` sidecar。改来源块格式时同步 `extract_tender_params.py`、prompt 与 `backend/tests/nodes/test_extract_tender_params_source_blocks.py`。

## Prompt 与 LLM 约定

- Prompt builder 放在 `backend/prompts/`，输入 DTO 放在 `backend/prompts/types.py`，返回 `RenderedPrompt`；不要在 node 或 service 中手写大段 prompt 拼接。
- 初次生成 prompt 统一走 `render_generate_prompt()`，由 `normalize_generation_style()` 把 `generation_style` 归一为 `template`（默认）或 `param`，再分派到 `render_generate_by_template_prompt()` 或 `render_generate_by_param_prompt()`。
...
- LLM 流式调用统一走 `backend/util/common_util/llm_stream_utils.py` 的 `stream_llm_completion()`；新增调用要复用 `StreamCallbacks`、`ensure_llm_env()`、`HeartbeatMonitor` 和 `LLM_STREAM_TIMEOUT_SECONDS`。
- `MODEL_CONFIGS` 中的 `deepseek`、`qwen`、`doubao` 是后端 provider 真源；新增 provider 必须同步 `LLMModel`、settings、测试和前端选项。
- Prompt、LLM 日志和 agent run 审计只能写 scrub 后摘要、配置键和节点名；不要写完整客户原文、真实路径、密钥值、traceback 或下载路径。

## 错误处理（error shape）

**模式：**
- API 层使用 `HTTPException`，`detail` 保持结构化字段：
  - `success: false`
  - `error.code`（稳定机器码，例如 `TASK_NOT_FOUND`、`COMMENT_SUPPLEMENT_NO_DOCUMENT`、`REQ_INVALID_PARAM`）
  - `error.message`（用户可读说明）
  - 可选 `error` 内附加字段（如 `task_id`）或顶层 `details`
  - `timestamp`（ISO 8601 UTC）
- OpenAPI 错误响应模型优先引用 `backend/models/common.py::ErrorResponse`（或端点局部的 `*ErrorResponse`，如 tender）。
- Pydantic validator 抛出 `ValueError`，由 FastAPI/Pydantic 负责请求错误响应。
- Service 层返回 `GenerateResponse` / `TaskResponse` 等模型，或在后台任务中把失败收敛为任务失败状态、SSE `error` 和进度日志。
- Graph 节点用 fail-fast 保护 Word 写入边界；受保护字段缺失、锚点缺失、非法字段区间、空 rewrite 正文等应抛出明确异常。
- Retrieval/embedding/Qdrant 失败降级为 `bm25_only` 并记录 warning，不阻塞批注生成。
- Agent run 预检失败返回 `needs_input` 事件，不创建后台任务；运行时异常收敛为 `error` 事件。
- LLM 超时使用 `LLMTimeoutError`；配置缺失由 `ensure_llm_env()` 抛出只包含配置键（如 `ARK_API_KEY`）的错误信息，不泄露任何值。
...
- 用户可见进度走 `backend/util/log_util/progress_log.py` 的 `progress_log` logger，通过 `QueueHandler` / `QueueListener` 和 `DailyFileHandler` 写入 `backend/logs/progress-YYYYMMDD.log`（目录由 `settings.LOG_DIR` 控制，默认相对 `backend/`）。
- 生成执行日志走 `backend/util/log_util/execution_log.py`。
- rewrite / task skill audit 写入 `backend/util/log_util/skill_audit_log.py`，落到 `backend/context_log/rewrite_log/<prefix>_<timestamp>_<audit_id>.json`（原子写 `_atomic_write_json`）。
- 支持的白名单 stage（`TASK_AUDIT_STAGES`）：`skill_directory_route`、`skill_prompt_render`、`rewrite_target_selection`、`rewrite_text`；非法 stage 会 `raise ValueError`。日志路径经 `task_audit_log_path` / 旧键 `rewrite_log_path` 注入 graph `configurable`。
- Agent run 审计由 `backend/agents/task_context_assistant/logging.py::AgentRunAuditLogger` 写 `backend/logs/agent-run-<run_id>.jsonl`。

**Agent run 审计白名单字段倾向：**
- 通用：`event`、`run_id`、`conversation_id`、`selected_skills`（空值剔除）。
- 按事件附加：`runtime`（`run_started`）；`stage`/`label`/`status`/`summary`/`guard_result`/`tool_name`/`selected_skill`（`thinking_stage`）；`tool_name`/`status`/`summary`/`task_kind`（`tool_call`）；`task_id`/`task_kind`/`status`/`queue_position`/`waiting_count`（`task_accepted`）；`summary`/`missing_requirements`/`selected_skill`（`needs_input`）；`summary`/`task_id`/`selected_skill`（`done`）；`code`/`summary`（`error`）。
- 用户可见/可记录摘要必须先经 `scrub_sensitive_text()`。

**`scrub_sensitive_text()` 规则（`backend/agents/task_context_assistant/logging.py`）：**
- 含 `Traceback (most recent call last):` → 整体替换为 `[REDACTED_STACK]`。
- 多行文本只保留首行后再 scrub。
- `Bearer ...` 与 `authorization|api_key|access_token|refresh_token|password|passwd|secret := <值>` → `[REDACTED_SECRET]`。
- Windows 绝对路径、Unix 敏感路径前缀（`/mnt|/home|/Users|/private|/var|/tmp|/etc/...`）、`.env` → `[REDACTED_PATH]`。
- **不记录**：完整客户原文、真实密钥/token、私有/绝对路径、traceback、下载路径、`backend/.env` 内容。
- 实现与测试：`backend/agents/task_context_assistant/logging.py`、`backend/tests/agents/test_task_context_assistant_logging.py`、`backend/tests/logging/test_task_audit_log_paths.py`、`backend/tests/nodes/test_skill_audit_log_path_resolution.py`。

## SSE 与 LLM 快照约定

- SSE 事件类型定义在 `backend/models/sse.py::SSEEventType`：`log`、`llm`、`progress`、`node_start`、`node_complete`、`agent_step`、`done`、`error`、`heartbeat`。
- 事件载体 `SSEEvent` 通过 `to_sse_format()` 输出标准 SSE 文本（`event:` + 多行 `data:` + 空行结束）。
- 各事件 data 模型：
  - `LogEventData`：`level` / `message` / `node`
  - `LLMEventData`：`content`、`content_mode`（默认 `snapshot`）、`is_complete`、`model`、`task_id`、`node`
  - `ProgressEventData`：`task_id`、`task_kind`、`status`、`completed_count`、`total_nodes`、`progress_text`、`progress_percent`、`current_node` / `current_node_display`
  - `AgentStepEventData`：`step_type`、`round`、`node`、`findings`，以及可选 `content_agent` / `comment_agent` 结构化过程
  - `DoneEventData`：`success`、`output_file`/`file_name`/`download_url`、可选 `style_writeback` / `comment_writeback` 摘要
  - `ErrorEventData`：`error`、`node`、`is_fatal`
  - `HeartbeatEventData`：`task_id`
- LLM 流式输出经 `DocumentService._LLMSnapshotRelay` 以**全文快照**（`content_mode="snapshot"`，非增量 chunk）形式经 `SSEEventType.LLM` 推送，按 `LLM_SNAPSHOT_INTERVAL_SECONDS` 节流；`node` 取自 `TASK_KIND_TO_LLM_NODE`，`is_complete` 标记终态。
- Agent step（参数生成 / 批注生成）结构化过程数据分别由 `ContentAgentStepData`、`CommentAgentStepData` 承载；新增阶段或字段须同步 model、写回 node、service 和前端展示。
- 相关测试：`backend/tests/services/test_document_service_llm_snapshot.py`、`backend/tests/services/test_sse_manager_agent_step.py`、`backend/tests/models/test_sse_agent_step.py`。
