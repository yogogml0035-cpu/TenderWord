...
**模板候选系统：**
- 模板候选列表 / 文件下载 / 选择落盘 - 后端代理外部模板系统，按年份阻断过旧模板，并限制下载主机白名单。
  - SDK/Client: `requests.get`
  - Auth: 未检测到单独认证字段；URL 和白名单来自配置
  - Config: `TEMPLATE_CANDIDATE_API_URL`、`TEMPLATE_CANDIDATE_ALLOWED_HOSTS`、`TEMPLATE_CANDIDATE_RANKING_LLM_PROVIDER`、`EXTERNAL_REQUEST_TIMEOUT_SECONDS`
...
- Agent run audit JSONL 默认位于 `backend/logs/`，实现点是 `backend/agents/task_context_assistant/logging.py`。
- 批注 bad case 源文件位于 `backend/retrieval/bad_cases/`，加载器是 `backend/retrieval/bad_case_loader.py`。
- 进度、执行、prompt、skill audit、SSE 日志由 `backend/util/log_util/` 管理，默认与 `LOG_DIR`、`backend/logs/` 相关。
