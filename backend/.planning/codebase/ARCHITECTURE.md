...
| RewriteSkillGraph | 显式声明 rewrite 节点顺序、普通边和条件分支（按 `rewrite_source` / `source_document_path` 分派） | `backend/graphs/skill_graph.py` |
...
| Comment agents | 批注候选首版生成/校验、工具门禁、确定性 Word 写回和审计 | `backend/agents/comments/comment_agent.py`, `backend/nodes/common_word_nodes/comment_agent.py` |
| Task context assistant | 右侧 agent run 前置流，只用受控上下文和白名单工具创建 rewrite 任务 | `backend/services/agent_run_service.py`, `backend/agents/task_context_assistant/tools.py` |
| Prompt layer | 只做 prompt 渲染和机器契约解析，不承载副作用、SSE、COM 或 session state | `backend/prompts/` |
| Retrieval layer | 为批注生成注入 bad case prompt context，hybrid 失败时降级 | `backend/retrieval/comment_bad_case_runtime.py`, `backend/retrieval/hybrid.py` |
...
- Used by: `backend/nodes/common_word_nodes/generate_comments.py`, `backend/nodes/common_word_nodes/comment_agent.py`, `backend/nodes/common_word_nodes/comment_supplement.py` 相关路径。
...
2. `DocumentService.create_task()` 使用 `request.form_type.value` 从 `GRAPH_REGISTRY` 选择 graph（`backend/services/document_service.py`）。`GRAPH_REGISTRY` 覆盖 `xjcg_tender`、`gngk_hw_zc_tender`、`gngk_hw_cz_tender`、`gngk_fw_zc_tender`、`gngk_fw_cz_tender`、`gjgk_tender`。
3. `_build_initial_state()` 将 tender data、文件路径、锚点、`generation_style`、`generation_mode`、`comment_generation_mode`、`style_writeback_mode`、`tender_lx`、`fund_source_lx` 写入初次 generate state。
4. `_submit_graph_task()` 调用 `TaskQueueManager.add_task()` 并把 `_run_graph()` 提交到后台线程池（`ThreadPoolExecutor`）。
5. `_run_graph()` 实例化 graph、估算节点总数、编译 graph、创建独立 event loop 并调用 graph `ainvoke`。
...
7. `StandardTenderWorkflowGraph.build_graph()` 执行：
   - `prepare_template -> extract_tender_params`
   - 并行进入 `word_operations_subgraph`（delete/get_replacements/replace 等类型差异步骤）与生成分支
   - `generation_mode_gate` 选择 `generate_polished_text`（workflow）或 `content_agent`（agent）
   - 两路最终正文汇合到共享 `annotate_corrections`（条款标识规范化 + 更正批注候选）
   - 再进入普通批注分支 / `comments_branch_done` 与 `update_word`
   - 最后按 `_select_after_update_node` 决定是否走 `comment_agent`
8. `annotate_corrections` **仅首次 generate 接入**；`RewriteSkillGraph` 不接入该节点。
9. `update_word`（含 gjgk/gngk 特化写回）先写 `correction_comments` 再写普通 `polished_comments`；`suppress_ai_comment_writeback` / `comment_generation_mode=off` 只跳过普通 AI 批注，不跳过更正批注。
10. `DocumentService` 收敛 output file、file size、model、style/comment writeback summary，并通过 task queue 与 `SSEManager` 推送 `done` 或 `error`。
...
2. `AgentRunService` 先输出 `run_started`、`thinking_stage`，再做 preflight guard 或调用 DeepAgents runner。
3. 条件缺失时返回 `needs_input`（**不创建任务**）；条件满足时由 `create_rewrite_task_tool()` 调用 `DocumentService.create_rewrite_task()`。
...
5. `RewriteSkillGraph.build_graph()` 拼装 6 个节点：`resolve_rewrite_target`、`extract_rewrite_context`、`get_rewrite_comments`、`delete_section`、`rewrite_text`、`update_word`；`delete_section` / `rewrite_text` 统一汇入 `update_word`（`backend/graphs/skill_graph.py`）。
6. 条件分支由 `backend/skills/rewrite/scripts/runtime.py` 决定：
   - `resolve_rewrite_target` 在 `rewrite_source=="uploaded_file"` 时走 `extract_rewrite_context`，否则直奔 `rewrite_text`
   - `rewrite_text` 后在 `uploaded_file` 或无有效 `source_document_path` 时跳过 `get_rewrite_comments` 直达 `delete_section`
...
8. rewrite 后台任务复用 task/SSE/download 链路；agent run 只负责 `task_accepted` 和终态摘要。
...
- `gngk` 在前端是 UI 类型；提交到后端时必须由共享 helper 按 `tender_lx` + `fund_lx`/`fund_source_lx` + `ifzgcg` 分派到具体 form type（`gngk_hw_zc/hw_cz/fw_zc/fw_cz`）。`get_tender_type_family()` 把这些子类型归并为 `gngk` 共享行为族（`backend/config/tender_config.py`）。
...
**Word 抽取保真（特殊字形 / 自动编号）：**
- 职责： 技术参数与模板抽取时保留可见编号与特殊字形，写回时无损恢复。
- Examples: `backend/util/word_util/word_extraction_utils.py`, `backend/util/word_util/word_symbol_tokens.py`, `backend/helper/word_helper/content_ops.py`
- Pattern: 未知字形用 `[[WORD_SYMBOL:...]]` token 进 prompt；自动编号经 `ListFormat` 前缀进抽取文本；字体恢复失败不得静默交付。

**Content agent / Comment agent:**
- 职责： 自主生成正文和批注锚点校验/写回。
- Examples: `backend/agents/generation/content_agents.py`, `backend/agents/comments/comment_agent.py`
- Pattern: agent 的长正文、草稿、审核和修订通过 workspace 文件交接；Word 写回仍由 graph 节点线程执行。`[[TABLE:<id>]]` 是内部写回入口，不是用户可见正文。
...
- Pattern: 提取阶段写入 `tender_param_table_models` 侧车；content agent 保留占位符；写回层按 sidecar 恢复 merge/真实表格。`verify_agent_graph` 与 `revise_agent_graph` 共同强制 `generation_style` 相关锚点表规则。
...
- Triggers: `python -m uvicorn main:app --reload --port 8000`（在 `backend/` 下）或 `uvicorn backend.main:app`。
...
- **Threading:** FastAPI 主 loop 处理 HTTP/SSE；`DocumentService` 用 `ThreadPoolExecutor` 提交任务；每个 graph 任务在后台线程中创建独立 asyncio loop。
...
- **Global state:** 单例包括 task queue、`sse_manager`、document/conversation/agent_run service、`settings` 和 graph registry。
