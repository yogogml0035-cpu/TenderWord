...
│   ├── generation/             # content_agent、generate/verify/revise 子图、workspace、sanitizer、guarded
...
├── helper/
│   └── word_helper/            # Word 业务 helper（含结构化表占位符解析）
├── models/                     # Pydantic API/runtime 模型
├── nodes/                      # LangGraph 节点
│   ├── common_word_nodes/      # 共享 Word/LLM/agent 节点与共享抽取能力
...
├── skills/
│   └── rewrite/                # rewrite task skill 声明与 runtime helper（分支判定）
...
- 关键文件： `backend/models/generate.py`, `backend/models/task.py`, `backend/models/sse.py`, `backend/models/agent_run.py`, `backend/models/tender.py`, `backend/models/upload.py`, `backend/models/template_candidates.py`, `backend/models/common.py`

**`backend/config/`:**
- 职责： 运行配置和招标类型配置。
- 包含： `settings.py`, `tender_config.py`
- 关键文件： `backend/config/settings.py`, `backend/config/tender_config.py`（`TenderAnchorConfig`、`ProtectedFieldProfile`、`get_tender_type_family`、锚点表 `ANCHOR_CONFIGS`）
...
- 关键文件： `backend/services/document_service.py`（`GRAPH_REGISTRY`、`create_task`/`create_rewrite_task`/`create_comment_supplement_task`、`_build_initial_state`、`_submit_graph_task`、`_run_graph`、`_invoke_graph_async`）, `backend/services/agent_run_service.py`, `backend/services/conversation_service.py`, `backend/services/template_candidate_ranking_service.py`

**`backend/task/`:**
- 职责： 长任务队列、状态、进度、取消、心跳、后台清理和公平执行。
- 包含： `task_queue_manager.py`
- 关键文件： `backend/task/task_queue_manager.py`（`TaskQueueManager`、`Task`、公平锁、cancel 传播）
...
- 关键文件： `backend/graphs/base_graph.py`（`CrossProcessFileLock`、`wrap_node_with_progress`、`StandardTenderWorkflowGraph.build_graph`）, `backend/graphs/skill_graph.py`（`RewriteSkillGraph` + 条件分支）, `backend/graphs/comment_supplement_graph.py`
...
- 关键文件： `backend/states/base_state.py`, `backend/states/skill_state.py`（`rewrite_source`/`rewrite_user_prompt`/`source_document_path`）, `backend/states/gngk_tender_state.py`
...
- 包含： `gngk_hw_zc`、`gngk_hw_cz`、`gngk_fw_zc` 的 replacement、delete、update 实现，以及共享的 `gngk_get_replacements.py`。
...
- 职责： 国际公开类型差异节点。
- 包含： 国际公开抽取、删除、替换、写回。
- 关键文件： `backend/nodes/gjgk_word_nodes/gjgk_update_word.py`, `backend/nodes/gjgk_word_nodes/gjgk_get_replacements.py`, `backend/nodes/gjgk_word_nodes/gjgk_delete_tender_param.py`, `backend/nodes/gjgk_word_nodes/gjgk_replace_content.py`
...
- 关键文件： `backend/nodes/skills_nodes/rewrite_nodes.py`（`resolve_rewrite_target`、`extract_rewrite_context`、`rewrite_text`）, `backend/nodes/skills_nodes/tender_aware_word_dispatch.py`（`dispatch_tender_aware_delete_section`、`dispatch_tender_aware_update_word`）

**`backend/helper/word_helper/`:**
- 职责： Word 业务 helper，适合放跨节点复用逻辑。
- 包含： 正文写回、段落边界、受保护字段、样式回填、删除、cleanup、range、语义匹配、结构化表占位符解析（sidecar 模型）。
- 关键文件： `backend/helper/word_helper/content_ops.py`, `backend/helper/word_helper/inline_style_ops.py`, `backend/helper/word_helper/protected_fields.py`, `backend/helper/word_helper/paragraph_boundary_ops.py`, `backend/helper/word_helper/text_parsing.py`（`[[TABLE:<id>]]` 契约与 sidecar 恢复）

**`backend/util/word_util/`:**
- 职责： Word COM 技术工具层。
- 包含： COM app 生命周期、COM lock/retry、Word 常量、锚点工具、底层插入、文档检查、诊断。
- 关键文件： `backend/util/word_util/word_application_util.py`（Word COM 创建、打开、保存、关闭）, `backend/util/word_util/word_com_manager.py`（`com_lock`/retry）, `backend/util/word_util/anchor_utils.py`, `backend/util/word_util/word_diagnostics.py`
...
- 关键文件： `backend/agents/generation/content_agents.py`, `backend/agents/generation/generate_agent_graph.py`, `backend/agents/generation/verify_agent_graph.py`, `backend/agents/generation/revise_agent_graph.py`, `backend/agents/generation/table_placeholder_utils.py`, `backend/agents/generation/content_sanitizer.py`, `backend/agents/generation/protected_field_guard.py`, `backend/agents/generation/workspace.py`, `backend/agents/generation/model_factory.py`

**`backend/agents/comments/`:**
- 职责： 批注 agent 运行时。
- 包含： LangChain agent、工具调用限制、引用校验、Word 写回工具、类型、审计 workspace。
- 关键文件： `backend/agents/comments/comment_agent.py`, `backend/agents/comments/tools.py`, `backend/agents/comments/types.py`, `backend/agents/comments/workspace.py`

**`backend/agents/task_context_assistant/`:**
- 职责： 右侧聊天 agent run 前置流。
- 包含： DeepAgents factory、rewrite 工具、只读摘要工具、scrub 审计日志。
- 关键文件： `backend/agents/task_context_assistant/factory.py`, `backend/agents/task_context_assistant/tools.py`（`create_rewrite_task_tool` 校验 `file_path`/`form_type`/`tender_lx`/`fund_source_lx`）, `backend/agents/task_context_assistant/logging.py`（`scrub_sensitive_text`、白名单摘要）
...
- 关键文件： `backend/retrieval/comment_bad_case_runtime.py`, `backend/retrieval/bm25.py`, `backend/retrieval/qdrant_store.py`, `backend/retrieval/hybrid.py`, `backend/retrieval/embeddings.py`, `backend/retrieval/bad_case_loader.py`, `backend/retrieval/config.py`

**`backend/util/common_util/`:**
- 职责： 非 Word 的通用工具。
- 包含： 上传落盘、模板候选代理、招标详情代理（解析 `tender_lx`/`fund_lx`/`ifzgcg`）、LLM stream、招标编号归一化。
- 关键文件： `backend/util/common_util/upload_storage.py`, `backend/util/common_util/template_candidates.py`, `backend/util/common_util/fetch_tender_data.py`（`tender_lx`/`fund_source_lx`/`ifzgcg` 归一化）, `backend/util/common_util/llm_stream_utils.py`
...
- 关键文件： `backend/util/log_util/progress_log.py`, `backend/util/log_util/execution_log.py`, `backend/util/log_util/sse_log_handler.py`, `backend/util/log_util/skill_audit_log.py`, `backend/util/log_util/log_cleanup.py`

**`backend/skills/`:**
- 职责： task skill 声明和 runtime helper。
- 包含： `catalog.py`, `rewrite/SKILL.md`, `rewrite/scripts/runtime.py`
- 关键文件： `backend/skills/rewrite/SKILL.md`, `backend/skills/rewrite/scripts/runtime.py`（`select_resolve_branch`/`select_comment_branch`/`estimate_total_nodes`/`has_source_document`）, `backend/skills/catalog.py`
...
**Entry Points:**
- `backend/main.py`: FastAPI app、router 注册（`/api` 前缀）、CORS、startup/shutdown、健康检查（`/health`、`/health/ready`、`/health/live`）、全局异常处理。
...
- `backend/config/tender_config.py`: 招标类型锚点、字号、content mode、受保护字段 profile、`get_tender_type_family`。
- `backend/requirements.txt`: 后端依赖清单。
- `backend/.env.example`: 示例环境配置；不要引用真实值。
- `backend/.env`: 本地私有配置文件存在；不得读取内容。

**Core Logic:**
- `backend/services/document_service.py`: generate/rewrite/comment supplement 任务创建、graph 执行、结果收敛；`GRAPH_REGISTRY` 在此注册 6 种 form type。
- `backend/task/task_queue_manager.py`: 任务队列、进度、取消、心跳、公平锁。
- `backend/graphs/base_graph.py`: graph 基类、标准生成主干、锁、进度、取消。
- `backend/graphs/skill_graph.py`: rewrite 显式 LangGraph 实现（条件分支按 `rewrite_source`/`source_document_path`）。
...
- `backend/nodes/common_word_nodes/replace_content.py`: 共享模板替换节点（`project_content`/`project_content_v2` 同路径回填）。
- `backend/nodes/common_word_nodes/get_replacements_shared.py`: 公共招标正文抽取共享能力（`extract_public_tender_project_content_v2`、labeled line extractor/formatter）。
...
- `backend/helper/word_helper/text_parsing.py`: `[[TABLE:<id>]]` 占位符契约与 sidecar 表格恢复。
...
- `backend/agents/generation/content_sanitizer.py`: 正文净化规则。
- `backend/agents/generation/protected_field_guard.py`: 受保护字段不可删除守卫。
- `backend/agents/comments/comment_agent.py`: comment agent 主运行时。
- `backend/agents/task_context_assistant/tools.py`: agent run 受控工具（含 `create_rewrite_task_tool`）。
- `backend/agents/task_context_assistant/logging.py`: `scrub_sensitive_text` 与白名单摘要。
