...
from typing import Any, Dict, Iterable, Literal, Optional, Sequence, TypedDict
...
def apply_inline_style_fragments(
    *,
    ...
    if int(bound_end) <= int(bound_start):
        log_parts.append(f"{step_label}：样式回填范围为空，跳过。")
    ...
    if not fragments:
        log_parts.append(f"{step_label}：未提取到可回填的行内样式。")
        return result

    target_containers = _build_target_containers(doc, bound_start=bound_start, bound_end=bound_end)
    log_parts.append(f"{step_label}：开始回填行内样式，共 {len(fragments)} 个片段。")
    ...
    log_parts.append(summarize_style_writeback_result(result))
    return result
...
def summarize_style_writeback_result(result: InlineStyleWritebackResult) -> str:
    ...
    if applied_by_style:
        ordered_styles = ", ".join(
            f"{style}={count}" for style, count in sorted(applied_by_style.items())
        )
