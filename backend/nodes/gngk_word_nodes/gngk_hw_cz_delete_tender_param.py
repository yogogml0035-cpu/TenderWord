...
def gngk_hw_cz_delete_tender_param(
    ...
    """按财政货物锚点删除第四章与第五章之间的正文区间。"""
    del config
    start_time = time.monotonic()
    print(f"[{NODE_NAME}] 开始执行...")
    _visible_log("开始删除 gngk_hw_cz 原始招标需求")
    ...
    before_text = state.get("insertion_before_text") or default_before_text
    after_text = state.get("insertion_after_text") or default_after_text
    ...
    try:
        ...
        _visible_log("文档打开完成，准备定位招标需求锚点")
