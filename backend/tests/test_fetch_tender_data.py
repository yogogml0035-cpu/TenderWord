...
def test_fetch_tender_data_normalizes_gjgk_project_number_from_tender_no(monkeypatch):
    monkeypatch.setattr(
        fetch_tender_data_util.settings,
        "TENDER_DATA_API_URL",
        "http://example.com/tender",
    ...
    def mock_get(url, params=None, timeout=None):
        assert url == "http://example.com/tender"
        assert params == {"tenderno": "0811-264DSITC0639"}
        assert timeout == 23.0
        return DummyResponse(
            json_payload={
                "data": {
    ...
    result = fetch_tender_data_util.fetch_tender_data("0811-264DSITC0639")

    assert result["data"]["project_number"] == "264DSITC0639"
    assert result["type"] == {
        "tender_lx": 0,
        "purchase_method": 0,
        "fund_lx": 1,
