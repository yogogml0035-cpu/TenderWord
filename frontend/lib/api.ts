...
export type SSEEventHandler = {
  onConnected?: (data: { task_id: string; message: string }) => void;
  onLog?: (data: { timestamp: string; level: string; message: string; node?: string }) => void;
  onLLM?: (data: { timestamp: string; node: string; content: string; is_complete: boolean; token_count?: number }) => void;
  onProgress?: (data: { timestamp: string; node: string; completed_count: number; total_nodes: number; progress_percent: number; current_node_display: string }) => void;
  onStatus?: (data: { timestamp: string; status: TaskStatus; message?: string; result?: unknown }) => void;
  onError?: (data: { timestamp: string; error_code: string; message: string; details?: string }) => void;
  onDone?: (data: { timestamp: string; task_id: string; status: TaskStatus; total_time_seconds: number }) => void;
  onHeartbeat?: (data: { timestamp: string; task_id: string }) => void;
};
...
export function createSSEConnection(
  ...
  const url = `${API_BASE_URL}/api/stream/${encodeURIComponent(taskId)}`;
