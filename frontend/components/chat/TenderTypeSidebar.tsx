...
export function TenderTypeSidebar({ onNewChat }: TenderTypeSidebarProps) {
  ...
  return (
    <div className="w-16 bg-[var(--background)] border-r border-[var(--border)] flex flex-col items-center py-4 h-full">
      ...
      <div className="flex flex-col gap-2">
        {tenderTypes.map((type) => (
          ...
          >
            <button
              className={cn(
                'w-12 h-12 rounded flex flex-col items-center justify-center gap-1 transition-colors duration-200',
                currentType === type.id
            ...
            {hoveredType === type.id && (
              <div className="absolute left-full top-0 ml-2 z-50">
                <div className="bg-[var(--background)] rounded shadow-lg border border-[var(--border)] p-3 min-w-[160px]">
                  ...
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-2 px-3 py-2 bg-[var(--primary)] text-white text-sm rounded hover:bg-[var(--primary)]/90 transition-colors"
                  >
                ...
                {/* Arrow */}
                <div className="absolute left-0 top-4 -ml-1 w-2 h-2 bg-[var(--background)] border-l border-b border-[var(--border)] transform rotate-45"></div>
              </div>
            )}
          </div>
        ))}
          ...
          <div className="flex flex-col gap-1">
            {conversations.slice(0, 5).map((conv) => (
              <button
                ...
                className={cn(
                  'w-full p-2 rounded text-[10px] text-left truncate transition-colors',
