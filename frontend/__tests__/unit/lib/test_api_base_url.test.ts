...
describe('apiBaseUrl helpers', () => {
  ...
  it('parses comma separated API base URL candidates', () => {
    expect(
      parseApiBaseUrlCandidates(' http://localhost:8000/ , , http://10.11.1.99:8000 ')
    ).toEqual(['http://localhost:8000', 'http://10.11.1.99:8000']);
  });
  ...
  it('prefers the configured candidate that matches the current host', () => {
    ...
    expect(
      resolveApiBaseUrl({
        raw: 'http://localhost:8000,http://10.11.1.99:8000',
        location,
      })
    ).toBe('http://10.11.1.99:8000');
  });
