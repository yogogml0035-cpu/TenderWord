...
def delete_tender_param(state, config):
    """
    根据前后内容定位插入位置，删除该位置的 WPS/Word 内容（GNGK版本）。
    
    从 state 中读取：
    - insertion_before_text: 插入位置的前置文本
    - insertion_after_text: 插入位置的后置文本
    - prepared_doc_path: WPS/Word 文档路径
    - tender_type: 招标类型（用于确定字号）
                ...
                if (
                    ...
                    while step_idx < max_steps:
                        ...
                        if after_start_pos > doc_end or current_pos < 0 or current_pos > doc_end:
                            print(f"⚠️ 位置异常，停止删除。current_pos={current_pos}, after_start_pos={after_start_pos}, doc_end={doc_end}")
                            break
                        ...
                        try:
                            if step_idx % 10 == 1:  # 每 10 步打印一次进度
                                print(f"  步骤 {step_idx}: 尝试删除 {current_pos} -> {after_start_pos} (大小: {after_start_pos - current_pos})")
                            t0 = time.time()
                            delete_rng.Delete()
...
if __name__ == "__main__":
    ...
    print("=" * 80)
    print("开始测试 delete_tender_param 节点")
    print("=" * 80)
