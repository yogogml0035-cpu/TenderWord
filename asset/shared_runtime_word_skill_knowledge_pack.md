...
- `comment_agent` 不再让模型做二轮锚点修复：模型只生成首版批注候选，代码负责确定性定位和写回。工具提交仍是首选，但模型未调用工具时，只要普通回复里输出了可解析的纯 JSON 数组候选，runner 也会复用同一套确定性校验与 Word 写回；`validate_comment_references` 的 `ToolCallLimitMiddleware` 收敛为 `run_limit=1` 且保持 non-fatal `continue`，第二次校验调用只会被拦住并写入审计，不再抛错阻断任务；`initial_comments` 非空（非自主生成）时直接跳过 runner，对现有候选做确定性校验和写回。
- 批注写回统一收敛到 `backend/nodes/common_word_nodes/comment_writeback.py::write_polished_comments()`：workflow、agent generate、comment_supplement、`comment_agent` 工具写回 (`write_validated_comment_candidates_to_word`) 都复用同一套 helper，`comment_agent` 不再保留独立的 `_find_word_anchor_ranges()`。重复锚点不再直接失败：共享写回层对所有未批注的真实重复位置分别写入同一条批注；默认 `allow_existing_comments=False` 时已有批注的位置计入 `overlapping_comment_exists` 跳过，`comment_agent` 写回显式 `allow_existing_comments=True` 允许同锚点追加，找不到目标才计入 warning/failed。`validate_comment_reference_candidates` 中“正文中出现多次”改为可写入状态（`reference_text_non_unique_will_expand_on_writeback`），不再进入 failed。
...
- `content_verify_agent` 的事实真源必须与当前 `generation_style` 的生成 prompt 保持一致：`project_info` 只为明确标注或可无歧义定位的同一项目级语义槽位提供项目名称、数量、交付和付款等基础事实；无字段标签的分隔片段只能在 `tender_params` 没有该槽位值时补充项目级字段，不能靠猜测覆盖明确技术参数，项目名称也不得推断为设备名称、维保设备、服务名称或采购标的名称。`tender_params` 是技术参数、重要性标识（紧邻编号前后的 `★/▲/△/Δ/*/#/※/●`，例如 Symbol 字体抽取出的 `Δ`）、包件数量、业务要求、表格 schema、条款物理顺序和文本/表格容器的原材料事实真源。生成与审核都要先建立“包/对象/来源字段标签/目标语义槽位/原始值”账本；允许模板给同一槽位换字段标签或把复合值无损拆格（如 `维保设备→设备名称`、`1套→数量1+单位套`），除此之外，名称限定词、同义改写、简称展开、数字写法、数值/单位/范围、频次、期限、地点、条件、否定词和比较符号的任何变化都必须阻止或报 finding。`template_reference_text` 只是受限基础格式真源，只能提供一级章节壳、基础信息字段壳、编号标点、冒号、占位符和通用排版线索，不能作为技术/服务/商务/售后参数正文、旧表格、旧重要性标识或旧子章节的审核依据。`template` 和 `param` 都必须审核项目概述、总体需求等基础信息结构的字段位置和容器是否被保留并用同槽位新事实替换；参数章节内部必须按当前生成风格校验技术/服务/商务/售后条款接管、旧标题粉碎、无源旧事实删除、旧表壳清洗和连续重编号。审核必须检查技术参数中重要性标识不能缺漏、额外增加或改变归属，且多包件/多标段原材料不能只生成一个包件。正文技术符号（`≥/±/×/Ω/SpO₂/℃`）按参数文本原样保留，不作为重要性标识参与审核。
- workflow/agent 最终正文写回前统一经过 `annotate_corrections`：代码先规范条款标识并为规范化生成更正批注；主 LLM 在同一 prompt 中接收 `tender_params` 与 `polished_text`，按同包、同对象、同语义槽位生成 `原技术参数为“aaa”，现改为“bbb”` 候选。独立的低温 LLM 复核候选，只返回可保留的序号；编号/层级/项目符号、空白/换行/末尾标点、模板字段壳和无损拆格必须剔除，不确定也剔除。`*`/`※→★`、`△`/`Δ→▲` 是必须保留的条款重要性标识更正；代码已经生成的同位置标识批注注入主 prompt，避免重复。项目名称不得授权设备/维保设备/服务/采购标的名称。代码只保留 JSON、固定句式、原值存在于 `tender_params`、现值与锚点存在于 `polished_text` 的机器契约，不再用正则推断条款编号或项目符号语义。该节点把主 prompt/原始输出、复核 prompt/原始输出和最终接受的更正批注分别写入 `backend/context_log/generate_log/correction_<task_id>_*_{prompt,raw_output,review_prompt,review_raw_output,accepted_comments}.txt`。普通 `comment_agent` 只看 `polished_text` 做合规/公平/严谨性审查，不得猜测原技术参数差异，避免与专用差异标注器重复或产生伪差异。
- `[[TABLE:<id>]]` 占位符是结构化表的内部写回入口，不是最终正文可见内容：生成 prompt 要求模型对采购需求结构化表保留锚点、删除紧邻锚点前的投影表行，不手绘/散文化表格；评分/评审表的投影和锚点仍整体删除。锚点只代表紧邻它之前的一张表格本体，不能吞掉表格前后的附件标题、报价说明、注释、签字盖章、日期或后续章节；`content_verify_agent` 必须把“多个锚点集中挪到文末导致邻接正文缺失”报为 finding，`content_revise_agent` 只恢复 audit 指定的非表格正文和锚点位置，不重画结构化表。写回层（`backend/helper/word_helper/text_parsing.py:convert_lines_to_items`）按 `tender_param_table_models` sidecar 决定是否恢复真实结构化表：命中 sidecar 时恢复为 `structured_table`，未命中时静默丢弃该占位符行（绝不作为可见文本写入 Word）；占位符前的 Markdown/pipe 投影表若无法匹配 sidecar，连同占位符整段丢弃，避免把近似/编造数据写入 Word。`table_placeholder_utils` 只保留 `extract_table_placeholders`、`find_missing_table_placeholders`、`find_required_table_placeholders` 三个识别 helper，不再有 `raise_if_table_placeholders_missing` / `restore_missing_table_placeholders` / `build_missing_table_placeholder_findings`。
...
- Word OOXML 抽取层 `backend/util/word_util/word_extraction_utils.py` 必须保留所有特殊字形：已知 Symbol / Wingdings 3 字形映射为可见 Unicode（如 `Δ`、`▲`），同时兼容 legacy 低字节码和 OOXML `F0xx` 私有区码；其它 `w:sym` 或 legacy symbol-font 文本 run 以可逆 `[[WORD_SYMBOL:...]]` token 进入 prompt，绝不静默删除。共享 `word_symbol_tokens` 在 Word 写回前恢复原字符并重新应用原字体；字体恢复失败必须中止写回，不得用不可见私有区字符静默交付。该约束覆盖正文段落、表格 cell 和结构化表 prompt context。
- 同一抽取层须恢复 Word/WPS 段落自动编号可见文本：`extract_text_with_list_numbers` 读取 `ListFormat`/`ListString`（失败时可回退 `ListValue`），正文已以前缀形式包含编号则不再重复拼接；丢失自动编号会导致参数条款在 prompt 中无号，进而干扰审核与编号隔离语义。
- 字段替换链路（`replace_content`）遵守 Word Find 查找串长度上限 `WORD_FIND_TEXT_MAX_LEN=256`：超长串跳过并写入 `replacement_log`，配合 `replacement_fields` 字段名与 `get_replacements_core` 优先级分组，避免超长项目内容误伤或 COM Find 失败。
