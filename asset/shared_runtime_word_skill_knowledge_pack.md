...
- workflow/agent 最终正文写回前统一经过 `annotate_corrections`：代码先规范条款标识并为规范化生成更正批注，低温 LLM 再结合 `project_name` / `project_content`，按同包、同对象、同语义槽位逐字比较 `tender_params` 与 `polished_text`，对可确定的参数文字变化生成 `原技术参数为“aaa”，现改为“bbb”` 批注；纯编号、布局、模板字段壳、复合值无损拆格、系统来源标记、评分污染删除和同槽位项目基础信息覆盖不标注，项目名称不得授权设备/维保设备/服务/采购标的名称。LLM 候选还必须通过固定句式、原值存在于 `tender_params`、现值与锚点存在于 `polished_text` 的代码门禁。普通 `comment_agent` 只看 `polished_text` 做合规/公平/严谨性审查，不得猜测原技术参数差异，避免与专用差异标注器重复或产生伪差异。
