...
- 普通聊天 prompt 由 `backend/prompts/chat_prompt.py` 渲染；service 层只负责调用 builder、执行 LLM 与 NDJSON 副作用，不得重新内联 system prompt 或历史窗口规则。
...
- 普通聊天 prompt：`backend/tests/prompts/test_chat_prompt.py`
