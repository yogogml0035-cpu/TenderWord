...
- `tender_type_identity_session_knowledge_pack.md`
  - 适用范围：招标类型 identity、`form_type` / `tender_type` / family、`gngk` 子类型分派、anchor 默认值、`ifzgcg` 对货物财政 graph 分派的影响、graph/node/replacement 特化、国内公开货物自筹替换字段边界、canonical URL、当前页面会话与草稿恢复。
