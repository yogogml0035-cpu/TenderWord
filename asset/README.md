...
- 改招标详情 API 数据契约、Prompt Layer、task skill、generate / rewrite / edit / comment_supplement runtime、`generation_mode` / `comment_generation_mode` 后端分流、DeepAgents content_agent、Word COM、SSE、任务结果透传、批注/样式回写或 `backend/helper/word_helper/` 时，优先读取 `shared_runtime_word_skill_knowledge_pack.md`。
