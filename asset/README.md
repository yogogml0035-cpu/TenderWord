...
- `shared_runtime_word_skill_knowledge_pack.md`
  - 适用范围：generate / rewrite / edit 运行时、用户流式 rewrite 路由、Prompt Layer、generate prompt 路由模式契约、模板生成的硬换行与冒号挂载列表约束、task skill runtime、task skill 输出范围契约、按参数生成时的无源引导段删除与重编号约束、Word COM、共享 Word helper、批注/样式回写、样式颜色门禁、任务结果与 SSE 透传。
