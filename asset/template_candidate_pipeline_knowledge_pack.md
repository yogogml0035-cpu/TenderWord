...
- 后端模板候选 API：`backend/tests/api/test_template_candidates_api.py`
- 后端模板候选工具：`backend/tests/util/test_template_candidates.py`
- 当前仓库没有独立的后端模板候选排序服务回归测试；若修改 AI 排序链路，应优先补到 `backend/tests/services/`。
