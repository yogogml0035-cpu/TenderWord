...
- `project_content` 必须按整行替换项目基础信息里的采购内容，例如 `便携式肌骨超声仪	壹台` 直接替换为 ERP `project_content`。公开招标 family 额外有 `project_content_v2`：它专门提取表格或段落里 `项目名称：` 与下一处 `招标人：/采购人：` 之间的旧裸值（如 `便携式肌骨超声仪/壹台`），并在 `project_name` 替换前用去标签后的 ERP `project_content` 替换；Word 表格文本中行号和字段间可能穿插 `\r` / `\x07` 单元格分隔符，v2 边界必须兼容。`设备名称及数量：...`、`采购内容：...` 仍按带字段壳整行替换。不得替换后续 `招标人`、地址、联系人等字段。不要恢复裸数量词全局替换；只替换 `壹台 -> 壹批` 会误伤技术参数或其它包件。`项目实施地点`、`项目实施时间`、`项目交付地点`、`项目交付日期` 等都应作为项目内容行后的边界。
