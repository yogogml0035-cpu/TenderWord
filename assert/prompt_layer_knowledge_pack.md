...
- 与 LLM 契约强绑定的固定字面量必须收口到 Prompt 层，包括 `rewrite`、`true` / `false` 语义、固定提示语和 force-rewrite 文案。
...
- rewrite prompt 输入：
  ...
  - 输出 `RenderedPrompt(system_prompt, user_prompt)`
- 路由 prompt 输入：
  - `RouteOrReplyPromptInput(messages, latest_user_message, latest_rewrite_state, has_rewrite_history, force_rewrite)`
  - 输出 `RenderedPrompt(system_prompt, user_prompt)`
- rewrite 目标选择输入：
...
- Prompt 层可以暴露 registry 和固定文案常量，但不能承担调用 `stream_llm_completion` 的职责。
...
- 建议验证命令：
  - `python -m py_compile backend/prompts/types.py backend/prompts/generate_prompt.py backend/prompts/comment_prompt.py backend/prompts/rewrite.py backend/prompts/routing.py backend/services/user_routing_service.py backend/nodes/common_word_nodes/generate_polished_text.py backend/nodes/common_word_nodes/generate_comments.py backend/nodes/skills_nodes/rewrite_nodes.py backend/tests/test_prompt_builders.py backend/tests/test_generate_comments.py backend/tests/test_user_routing_service.py`
  - `python -m pytest backend/tests/test_prompt_builders.py -q`
- 当前环境已知风险：
