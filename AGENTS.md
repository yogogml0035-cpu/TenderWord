...
- 修正完成后必须回看本轮是否产生通用性经验；只有能指导未来招标类型、graph、node、Prompt Layer、API/SSE、前端会话或模板候选链路的规则，才追加到对应 `asset/` 知识包。
- 经验写入遵循“原则性 > 数值性”：说明边界为什么会错、为什么必须这样收敛，而不是记录某个阈值、颜色、字段值、文案、文件名或一次性补丁结果。
- 经验写入遵循“模式级 > 页面/节点级”：抽象到会重复出现的架构模式、同步面或契约模式，例如类型 identity、任务状态、SSE 事件、Word 写回、Prompt 契约、模板候选代理等，而不是只描述某个页面、某个节点或某个 bug 的局部现象。
- 经验写入遵循“可迁移 > 可复制”：去掉具体类型名、页面名、数值、本地路径和临时样例后仍能指导决策，才算适合进入知识包；否则只留在变更说明、测试用例或代码注释中。
