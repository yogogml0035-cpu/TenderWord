...
# Agent 项目可迁移设计方法论沉淀规则

## 背景与范围

本规则用于所有 Agent 项目的 bug 修复、需求新增和行为调整后的经验沉淀。目标不是记录单次修复过程，而是判断修改逻辑中是否存在可迁移的设计方法论，并把它转化为可复用的架构设计、模块拆分、协作关系设计和方案取舍依据。跨项目通用资产经验沉淀必须落盘到 `D:\Assets\Agent_All_Asset`；在 WSL 环境中对应路径为 `/mnt/d/Assets/Agent_All_Asset`。不得写入仓库内临时目录、个人下载目录、任务工作区或其他未约定位置。

方法论应保持中度抽象：保留模块职责、关键协作关系、边界条件、状态流、失败处理和方案取舍；去掉参数、变量、字段值、客户数据、敏感配置、本机路径、临时样例和强项目绑定细节。

## 何时沉淀

- 每次修复 bug、新增需求或调整行为后，都要判断本次修改逻辑是否能提炼出可迁移的设计方法论。
- 如果可以，应写入对应项目知识包；如果影响跨项目 Agent 架构设计，应同步到通用资产或仓库级协作规则。
- 如果不能沉淀，应在交付说明中简要说明原因，例如仅修正文案、格式、测试夹具或单点数据，不涉及模块职责、协作边界或设计取舍变化。

## 方法论应回答什么

- 解决的问题：描述被修复的设计缺口、协作断点、边界缺失或用户路径问题，不记录临时排障过程。
- 为什么这样设计：说明当前方案背后的约束、取舍和稳定性来源，而不是只说采用了哪个实现。
- 模块如何拆分：说明哪些模块负责输入、状态、执行、持久化、展示、校验、外部集成或失败处理。
- 模块如何协作：说明模块之间如何传递状态、事件、权限、产物、错误和回滚信号。
- 设计原则与决策逻辑：沉淀可复用的边界、职责、契约和取舍规则，而不是记录局部实现细节。
- 可迁移经验：说明这条经验在其他 Agent 项目中可以如何复用，尤其是对架构设计、模块拆分、协作关系设计和方案取舍的指导。

## 抽象层级要求

- 有效的方法论不是流程复述，也不是“本次改了哪些文件”的项目化复盘。
- 不写参数值、变量名、字段值、页面名、客户数据、敏感配置、本机路径、临时样例或只能在当前项目照搬的细节。
- 不写空泛套话，例如“要提高可维护性”“要模块化”“要注意边界”；必须说明哪类边界为什么会错，以及应该如何拆分和协作。
- 判断抽象是否合格：去掉具体数值、页面名、字段名、函数名、项目名后，这句话仍能指导同类问题的决策；如果不能，就还停留在实现记录或项目复盘层。
...
- 原则性 > 数值性：写“为什么错”和“应遵守什么约束”，不要只记录“改了什么值”。反例：“焦点卡片 shimmer 用 rgba(217,119,87,0.12)”；正例：“accent 色卡片的自定义微光应与边框色系一致”。
- 模式级 > 页面级：写“哪种布局、状态流、工具调用或模块协作模式容易犯这个错”，不要只绑定某个页面、节点或组件。反例：“VS 对比页左右栏要用面对面原则”；正例：“双栏对比布局中，两栏应朝分隔线对齐，而非朝容器中心对齐”。
- 可迁移 > 可复制：写“换到另一个 Agent 项目是否仍能指导决策”，不要沉淀只能照搬文件名、类名或局部实现的经验。反例：“page14 的阶梯图 CSS class 要通用”；正例：“可复用的视觉组件应使用通用 CSS class 名”。
- 所有通用 Agent 项目的经验沉淀都应优先服务架构设计、模块拆分、协作关系设计和方案取舍；如果一条经验去掉具体数值和页面名后不能继续指导决策，就还没有抽象到可迁移层级。